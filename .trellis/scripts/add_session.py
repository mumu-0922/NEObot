...
def _auto_commit_workspace(
    ...
    """Stage and commit only the journal/index paths written by the caller.

    The explicit path list is also passed to ``git commit --only``. This keeps
    pre-existing staged changes out of the journal commit and leaves them
    staged for the developer's later commit. We never ``git add`` the whole
    ``.trellis/`` tree, and if ``.gitignore`` blocks the specific paths we warn
    and skip instead of retrying with ``-f``.
    ...
    paths = [
        ...
        for path in (journal_file, index_file)
        if path is not None
    ]
...
def add_session(
    ...
    if auto_commit:
        print("", file=sys.stderr)
        _auto_commit_workspace(repo_root, target_file, index_file)
