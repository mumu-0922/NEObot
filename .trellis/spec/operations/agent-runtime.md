...
- Migration `087` concurrent reservation, stale Parent/Child launch, terminal
  ...
  drills must peel the empty product tail and return to current head `095`.
...
- Every prior PostgreSQL tail drill must peel `091`, `090`, `089` and then `088`
  before testing its older guard and finish reapplied at head `095`.
...
- Verify every older Agent/MCP/Assistant/Skill PostgreSQL drill peels empty
  `090` before its original guard and returns to head `095`.
...
- All older Agent/MCP/Assistant/Skill PostgreSQL drills peel empty `091`, then
  empty `090`, before their original guards and finish at head `095`.
...
- PostgreSQL 17 gate: schema `001 -> 090`, sanitized fixtures, real full dump
  ...
  byte-equivalence, repeated expected-zero apply, and head `095`.
...
- A closure record binds one Git commit, migration head `095`, Runner manifest/
...
- G20.10 itself adds no migration, Compose/startup worker, production adapter,
  ...
  contract binds head `095`; the current host stays `ISOLATION_UNAVAILABLE`.
...
- Run every Agent source and PostgreSQL 17 gate through schema head `095`, then
...
- Preflight requires distinct exact identities, private Runner/relay endpoints,
  ...
  `broker_artifact_canary` record at migration head `095`.
...
- Preflight validates a strict one-action current-window plan, Ed25519-signed
  approval, current head `095`, stable activation binding fingerprint, exact
...
- Preflight validates migration head `095`, private exact Runner endpoint,
...
- Require fresh READY G21.0-G21.4 evidence, exact migration head `095`, exact
...
## Scenario: Deploy the G21.6 bounded product-canary profile

### 1. Scope / Trigger

Apply when configuring `agent-runtime-product-canary`, the thirteenth database
principal, seventh execution caller, nine secure mounts, product activation,
final closure/promotion, restore or rollback. The development host remains
ineligible.

### 2. Signatures

```bash
bash mm-chat/scripts/verify-agent-product-canary-activation.sh
bash mm-chat/scripts/verify-agent-product-canary-postgres17.sh
bash mm-chat/scripts/verify-agent-runtime-g21-6-preflight.sh
bash mm-chat/scripts/verify-agent-production-closure.sh
bash mm-chat/scripts/verify-agent-runtime-g21-6.sh
```

Profile: `agent-runtime-product-canary`. Caller:
`spiffe://neo-chat/agent-runtime-product-canary`. Principal:
`agent_product_canary_app` with exactly three inherited roles.

### 3. Contracts

- Require every G21.0-G21.5 stage flag and fresh same-release READY evidence,
  migration head `095`, one exact bounded activation and the reviewed fixed
  plan. Broad Runtime/Scheduler/Learning/Skill-install remain false.
- Use a distinct LOGIN, caller, client TLS/key/CA, release manifest, activation,
  plan and Ed25519 authority pair. Mount exactly nine regular owner-secure files
  read-only with host-path creation disabled. Policy may use the reviewed shared
  file; no other prior canary material may be reused.
- Provide only the product database URL and private literal HTTPS Runner tuple.
  Mount no Compose Secret and provide no S3, MCP, Provider, vault, Redis,
  Broker, Project, Child, Cron, Draft-learning, Promote or admin credential.
- Runner grants only Probe/List/Reconcile/Launch/Heartbeat/Cancel and receives
  no product database, activation, plan or private authority key. Sandbox is
  credential-free, depth zero and `networkMode=none`.
- Monitor content-free queue depth, remaining activation budget, stale claims,
  failures and exact activation Sandbox residue using the existing label
  allowlist. Health performs only read-only PostgreSQL stale/pending queries and
  Runner Probe/List; it requires no expired claim, pending terminalization or
  Sandbox residue and never reconciles state.
- After the immutable receipt exists, run the read-only closure evaluator over
  the same release, full activation chain and exact product binding. Only a
  separate operator may append `PROMOTION_READY`; template/offline/disposable
  evidence cannot promote.
- Rollback stops only this profile, disables its activation and reconciles exact
  residue. Preserve request, receipt, promotion, incident and audit facts. Do
  not down migration `095` in production.
- Restore with the profile off, treat claims/tokens as expired, resolve the same
  request/Run/Attempt, wait lease/authority expiry and prove Runner absence.
  Require a fresh activation and closure after restore.
- Clean-copy verification must exclude `.env.single-server`, `data/`,
  `secrets/` and `backup/` at the copy source. Excluding only the destination or
  merely avoiding later reads still traverses protected runtime state.

### 4. Validation & Error Matrix

| Condition | Required result |
| --- | --- |
| any prerequisite flag/evidence absent | preflight rejects before DB/Runner |
| reused identity/file/principal or extra role | preflight/startup rejects |
| public/hostname Runner or widened plan | reject before launch |
| product receives effect/admin credential | source/Compose gate fails |
| stale activation/claim or Runner residue | unhealthy; no new request |
| incomplete/drifted/residual closure | no `PROMOTION_READY` |
| clean-copy source includes a protected runtime path | verification fails before traversal |
| current development host | expected nonzero `ISOLATION_UNAVAILABLE` |

### 5. Good / Base / Bad Cases

- **Good:** the exact private profile starts only after all seven prior stages,
  produces one bounded receipt, reports read-only zero-residue health and feeds
  the separate closure evaluator/operator promotion path.
- **Base:** the profile and flag remain off, protected runtime paths are not
  copied or read, and the development host stays `ISOLATION_UNAVAILABLE`.
- **Bad:** make health reconcile, reuse prior stage credentials, copy `secrets/`
  into a disposable tree, or treat offline closure fixtures as promotion.

### 6. Tests Required

- Render default/development/production Compose and prove default-off,
  private-only, hardened, nine read-only mounts, no secret/port and production
  image-only wiring.
- Run exact enabled preflight plus prerequisite, reuse, held-evidence and
  forbidden-credential negatives.
- Run PostgreSQL activation/request/lease/receipt/promotion/ACL/dump-restore/
  down-up proof, activation and 16-check closure gates.
- Run Phase 0, G21.0-G21.5, full standalone and expected-nonzero exact-host
  acceptance. Every older tail drill peels `095` first and returns to head
  `095`.
- Inspect the clean-copy command itself and assert all four protected paths are
  source exclusions; a green test after copying them is not acceptable proof.

### 7. Wrong vs Correct

- **Wrong:** `tar` or `rsync` the product tree first and ignore protected paths
  only after extraction, or use a mutating reconcile endpoint for health.
- **Correct:** exclude all protected paths in the source copy command and keep
  health to PostgreSQL stale/pending reads plus Runner Probe/List.
