...
For the workflow state machine's local runtime, status resolution and writes
are implemented by `.trellis/scripts/common/active_task.py`,
`.trellis/scripts/common/task_store.py`, and `.trellis/scripts/task.py`. The
Codex per-turn parser is `.codex/hooks/inject-workflow-state.py`; it reads this
file and contains no duplicate workflow-state text.
