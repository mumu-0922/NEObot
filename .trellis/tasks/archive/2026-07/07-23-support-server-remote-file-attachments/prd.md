...
- [x] A public HTTPS text/image/audio URL imports and returns a normal server
      `fileId`, then sends through the existing attachment path.
- [x] Localhost, literal private IP, DNS-to-private, embedded credentials,
...
- [x] Content-Length above the limit fails before reading the body; chunked or
      dishonest responses are still stopped at limit + 1.
- [x] Imported metadata includes purpose, conversationId, and clientFileId.
- [x] Existing multipart upload, download, delete, local-mode adapters, native
      images, direct document parsing, and attachment-only sends remain green.
- [x] Backend tests, Go vet, frontend tests, lint, typecheck, format check, and
