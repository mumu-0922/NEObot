...
- The current runnable Next.js frontend and 25 legacy `/api/*` handlers still
  live at repository root; `mm-chat/frontend/` is not independently runnable.
- The owner requires `mm-chat/` to become the only independently runnable
  complete project directory and intends to delete the original root project
  only after migration is proven complete.
- The owner selected full user-visible feature parity rather than retiring the
  existing Plugin, Voice, Image, Code Execution, Agent, BYOK, Document, RAG, or
  Search capabilities.
...
- The owner selected a server-only final runtime. The legacy local runtime is a
  temporary migration/rollback source, not a permanent production mode; old
  browser data remains recoverable through an explicit import flow.
...
- Reimplement all legacy server capabilities behind the Go backend contract;
...
- [ ] `mm-chat/frontend/` is independently installable, testable, buildable, and
      runnable.
- [ ] `mm-chat/` has no required path/import/workspace/build/runtime dependency
      on the original root application.
- [ ] Every original frontend and `/api/*` capability is mapped to migrated,
      intentionally retired, or explicitly deferred status with owner approval.
- [ ] The frozen legacy feature inventory has full functional parity through
      Go-backed contracts; no existing user-visible capability is silently
      removed.
- [ ] Visual regression and interaction smoke tests prove that the migrated
      frontend preserves the existing interface across agreed desktop/mobile
      viewports.
- [ ] A clean-copy deployment using only `mm-chat/` passes the frozen parity,
      security, backup/restore, and rollback gates.
- [ ] Original-project deletion is performed only after a separate exact delete
      plan and one-shot owner confirmation.
...
**Product scope decision**: Full feature parity is required. The current
frontend UI/theme remains the product baseline; this refactor changes ownership
and runtime boundaries, not the established visual identity.

**Runtime decision**: The final application is server-only. Local mode remains
only as a bounded transition and rollback mechanism until Go parity, browser
data import, and clean-copy gates pass.
