...
* [ ] 选中含 `LINUX DO 注册申请小作文模板` 主题文档的 Knowledge 后，问题
      `有小作文模板嘛` 会触发 `search_knowledge`，并能产生有效 `[K#]` Citation。
* [ ] 同一会话提问无关常识/普通写作时不触发 Knowledge I/O。
* [ ] 模糊且无 catalog overlap、无私有资料信号、无 Knowledge 追问关系的问题不会因为
      “可能相关”而触发 Knowledge。
* [ ] 同一会话询问当前公开信息时仍可只走 Web，不被 Knowledge 抢占 authority。
* [ ] 混合问题可同时获得 Knowledge 与 Web evidence，并保持 `[K#]`、`[W#]` authority
      和 Citation 边界；单一来源问题不会为了“更多材料”默认双搜。
* [ ] Native Tool 首轮保持 `tool_choice=auto`，没有无条件 Knowledge-first。
* [ ] non-Tool compatibility 路径只有在同模型规划为相关时才执行 Knowledge。
* [ ] non-Tool unified planner 可选择 `direct|knowledge|web|both`，输出无效或失败时
      fail-open 为 direct/现有安全 fallback，不退化成每轮 Knowledge retrieval。
* [ ] Tool capability probe 不携带用户数据；supported/unsupported/unknown 分类、cache key
      隔离、配置变更失效、明确不兼容 runtime downgrade 与 transient failure 不误降级均有测试。
* [ ] Provider default 与 model override 的继承/优先级、非法 model ID、配置变更 cache
      invalidation 及前后端 round-trip 有测试。
* [ ] 后台预热、unknown 当前轮 Planner、singleflight 去重、probe cancellation/timeout 和
      请求主链不被后台 probe 阻塞均有测试。
* [ ] capability cache 多实例可见、TTL expiry、config hash 隔离、旧记录不命中及数据库失败
      时安全回退为 unknown/Planner，不阻断聊天。
* [ ] Planner invalid JSON、timeout、provider failure 分别覆盖 strong Knowledge signal、forced
      Web 与 no-signal Direct fallback，且不会默认 Both 或无条件 Knowledge retrieval。
* [ ] catalog 内容有严格总字节、单字段、collection 数、document 数上限，并标记为
      untrusted data。
* [ ] 未授权、已删除、非 active 或不在当前 selection 中的 collection/document 不进入
      catalog。
* [ ] catalog/governance 失败不会阻断普通聊天，也不会降级成无条件检索。
* [ ] diagnostics 不持久化 exact query、catalog titles/descriptions 或 Knowledge 正文。
* [ ] Knowledge miss、Web-only、Knowledge-only、mixed、cancellation、provider failure 测试
      保持通过。
* [ ] Knowledge miss 后，公开时效问题可继续 Web，私有文档存在性问题不把内部查询无条件
      发往 Web，且任何路径都不产生虚假 `[K#]`。
* [ ] 聊天 UI 可区分 Direct、Knowledge、Web、Both；展开理由经过 allowlist/redaction，
...
```json
{"route":"both","knowledgeQuery":"standalone private query",
