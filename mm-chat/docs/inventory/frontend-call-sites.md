...
| P2 | `src/utils/pluginUtils.ts` | via `pluginApi.execute`; local adapter retains `/api/plugins/execute` | Server adapter targets `/v1/plugins/execute`; G4.5c.2b sends id-only payloads and Go resolves built-ins/registered/custom OpenAPI plugins from memory/Postgres. |
