...
Current frontend behavior uses the typed API client for server-owned domains,
while a bounded set of Next and browser-local compatibility paths remains:
...
|       P2 | `src/services/api/pluginService.ts` | Removed by the MCP cutover                                                                                                                         | Replaced by typed `mcpApi`; no local adapter or OpenAPI install path.                                                                                                                                                                                                                                                                                                                                                                                       |
|       P2 | `src/utils/pluginUtils.ts`          | Removed by the MCP cutover                                                                                                                         | Tool calls execute only in the Go provider-native loop.                                                                                                                                                                                                                                                                                                                                                                                                     |
|       P2 | `src/services/api/voiceService.ts`  | Local mode retains `/api/voice/transcribe` and `/api/voice/synthesize`; server mode uses Go synthesis only | `voiceJobs.synthesizeVoice` posts `messageId + text` to `/v1/voice/synthesize`, then the File client downloads the authenticated artifact. Server STT and non-default/non-browser TTS choices fail closed without a legacy-route call. |
...
|       P3 | `src/services/api/skillService.ts`  | Removed by G20.9                                                                                                                                  | Legacy catalogs and browser prompt resolution were hard-deleted; Package Skills use the typed server boundary.                                                                                                                                                                                                                                                                                                                                                |
...
|       P1 | `src/lib/data/clearAppData.ts`       | clears the retired `knowledge-storage` key and OPFS area | Local reset only; server Knowledge deletion uses Go APIs | Server mode keeps browser cleanup separate from server data deletion.      |
|       P2 | `src/lib/plugin/serverRegistry.ts`   | Removed by the MCP cutover                               | `mcpApi` / server-authoritative MCP state                 | No browser registry or execution fallback.                                 |
...
|       P3 | `src/lib/utils/attachments.ts`       | `fetch(blobUrl)`                                         | local file resolver                                      | Blob/object URL fetches stay local; do not route to Go.                    |
...
|       P1 | `src/lib/data/appExport.ts`                  | reads retired `knowledge-storage` for explicit export     | Legacy migration input only; it is not runtime Knowledge authority.                               |
...
|       P1 | `src/lib/data/appExport.ts`, `browserImportPackage.ts` | reads legacy OPFS knowledge files for explicit export | Import boundary only; live Knowledge uses server `fileId`. |
...
| -------: | -------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------- |
|       P0 | `src/components/app/ChatApp.tsx`, chat hooks/stores indirectly | `chatService`, agent services                   | Main chat spine must wrap or replace `chatService` first; G20.9 removed legacy Skill prompt injection. |
...
|       P2 | `src/components/plugin/PluginMarket.tsx`                       | Removed                                         | Replaced by `src/components/mcp/McpToolsControl.tsx` and Go `/v1/mcp/*`.    |
...
|       P3 | `src/components/skill/SkillMarket.tsx`                         | Removed by G20.9                                | Legacy UI/static catalogs no longer exist; do not recreate a browser executor.         |
...
7. Keep remaining unavailable paths behind typed API-client capability gates.
   Voice now splits synthesis from transcription: hosted TTS uses Go, browser
   speech remains explicit/local, and STT stays closed. G16 retired browser-
   local RAG/doc-parse authority; live Knowledge uses Go plus the Python worker.
