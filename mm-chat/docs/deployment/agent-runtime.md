...
The ordinary Chat Agent event log and Goal authority advance the application
schema to migration `097`. This does not promote or rewrite the exact-host bundle: its reviewed
activation/closure artifacts remain pinned to `095` and therefore stay held.
Disposable legacy tail drills peel empty `097`, then `096`, before `095` and
reapply through the current head `097`.
