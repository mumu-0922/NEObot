...
| `AGENT_TIMELINE_ENABLED`                      | API-only typed Agent timeline switch; defaults false. It requires an exact canary match and does not stop durable event writes. |
| `AGENT_TIMELINE_CANARY_USER_IDS`              | API-only unique exact UUID allowlist; defaults empty and fail-closed. Invalid or duplicate UUIDs stop startup/preflight. |
