...
| User actions/answer links | `memory_user_actions`, `memory_user_action_targets`, `message_memory_activities`, `message_memory_usages` | Typed current-user actions, link-only Activity only while its owner-matching assistant exists, and immutable revision-pinned Usage. Durable jobs outlive deleted messages without weakening the Activity foreign key. |
