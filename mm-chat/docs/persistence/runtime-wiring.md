...
| `AGENT_TIMELINE_ENABLED` | Go API | No | Default `false`; enables typed Agent timeline presentation only for an exact canary UUID. Event writes and legacy ProcessStep projection continue while false. |
| `AGENT_TIMELINE_CANARY_USER_IDS` | Go API | No | Default empty; unique comma-separated authenticated UUIDs. Empty/malformed/duplicate/non-matching input fails closed. |
