...
> 运行时快照（2026-07-10）：`004_phase15_identity_knowledge_acl` 已提交 `teams`、`team_memberships`、`team_invites`、`knowledge_collections`、`processing_consents` 与 `knowledge_outbox` schema；Phase 15.1B identity services 已完成。Phase 15.1C 以 [`phase-15-1c-team-services-plan.md`](./phase-15-1c-team-services-plan.md) 为唯一设计源（独立审阅 `P0/P1/P2=0`），当前代码尚未提供 `/v1/teams*` Team runtime，也未落 last-admin/account-disable fence 与加密 `identity_mail_outbox`。
...
Owner 已确认目标是小团队：每个用户拥有自己的 Personal Knowledge，另有 Team Knowledge，由 Team Admin 管理。运行时基线已前进：migration `004_phase15_identity_knowledge_acl` 已落 `teams`、`team_memberships`、`team_invites`、`knowledge_collections`、`processing_consents` 与 `knowledge_outbox` schema，Phase 15.1B identity services 已完成。当前 Go Backend 仍未提供 `/v1/teams*` Team runtime、Membership 管理、Team Knowledge ACL 执行或 Invite Mail Outbox 发送链；15.1C Team service 行为一律以 [`phase-15-1c-team-services-plan.md`](./phase-15-1c-team-services-plan.md) 为准，因此下列模型描述的是目标运行时，不代表现有代码已完成。
...
Personal Collection 必须且只能绑定 `owner_user_id`；Team Collection 必须且只能绑定 `team_id`。`workspace` 继续表示聊天 UI 分组，不得静默等同 Team/IAM Scope。Team Membership Role 与全局 User Role 分离：运行时 Team authority 只认 `team_memberships.role` 与 DTO `teamRole`，`CurrentUser.role = "user"` 只是兼容元数据。系统至少保留一个 Active Team Admin；最后一个可用 Admin 的降级、删除、离队或 account disable 都必须事务化拒绝。
...
默认采用 Admin Invite、禁止公开注册、每用户独立 Session。Invite Acceptance 对新账号创建首个 Argon2id Password Credential；已存在的 Active 账号可加入第二个或后续 Team，但必须同时证明邀请 Mailbox Token 与当前 Password，且不得覆盖既有 Credential。Recovery Token 只发送到 Verified Mailbox，Team Admin 不得重置成员 Credential。Invite 创建与发送必须通过加密 `identity_mail_outbox` 原子提交；SMTP、加密或 durable sender admission 不可用时应 fail closed。若以后需要成员维护 Team Knowledge，新增 `contributor` 权限而不是扩大 `member`。请求必须显式提交非空 Collection ID 列表；Go 根据当前 User、Membership Revision 和 Collection Row 计算 Allowed Collections，拒绝客户端提供的 `user_id`、`team_id`、`acl_groups` 或 Impersonation Hint。Search Payload 和 Cache Key 使用 Contract 中规范化的 Collection/Document/User/Governance Fence。

15.1C Team API、Invite 事务/锁序与 account-disable fence 以 [`phase-15-1c-team-services-plan.md`](./phase-15-1c-team-services-plan.md) 为唯一设计源；Knowledge ACL 的权限矩阵、Consent、File Binding、Deletion 和测试要求由 [`knowledge-acl-api.md`](../contracts/knowledge-acl-api.md) 定义。两者都是 Phase 15 Contract，不代表当前 Backend 已实现。
