...
> 运行时快照（2026-07-10）：`004_phase15_identity_knowledge_acl` 已提交 Team/Membership/Knowledge ACL schema，Phase 15.1B identity services 已完成；Phase 15.1C 以 [`phase-15-1c-team-services-plan.md`](./phase-15-1c-team-services-plan.md) 为唯一设计源（独立审阅 `P0/P1/P2=0`），当前代码尚未提供 `/v1/teams*`、Team ACL 执行、last-admin/account-disable fence 或加密 `identity_mail_outbox`。
...
截至 2026-07-10，migration `004_phase15_identity_knowledge_acl` 已建立这些权威表，Phase 15.1B identity services 已完成；但 Team runtime、Collection ACL enforcement 与 Invite delivery durable path 仍未完成代码。15.1C Team 行为以 [`phase-15-1c-team-services-plan.md`](./phase-15-1c-team-services-plan.md) 为唯一设计源。

Personal Collection 必须且只能绑定一个 `owner_user_id`；Team Collection 必须且只能绑定一个 `team_id`。Team Membership 首批只设 `admin|member`：Admin 管理 Team Membership、Team Document 和 Team Collection Consent，但不得读取其他用户的 Personal Knowledge；Member 默认只能查询 Team Knowledge。运行时 Team authority 只认 `team_memberships.role` 与 DTO `teamRole`，全局 `CurrentUser.role = "user"` 只是兼容元数据。已存在的 Active User 可以通过 Invite 接受第二个或后续 Team，前提是同时证明 Invite Mailbox Token 与当前 Password，且不得覆盖既有 Credential；新 User 则在 Acceptance 时创建首个 Credential。需要成员维护 Team 文档时新增 `contributor`，不得静默扩大 Member 权限。系统必须始终保留至少一个 Active Admin；最后一个可用 Admin 的降级、删除、离队或 account disable 事务必须拒绝。
...
后台 Parse/Index 不依赖 Query Consent；Query Embedding 不能拿 Collection Consent 代替用户授权；Rerank/Answer 不能静默丢弃缺少 Consent 的 Collection。Invite 创建/发送必须与加密 `identity_mail_outbox` 同事务提交；Raw Token 只允许存在于有界进程内存与密文负载，不得出现在日志、响应或 Admin API。
