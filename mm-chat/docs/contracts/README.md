...
- [`agent-runtime.md`](./agent-runtime.md) — implemented G20.1 package admission, G20.2 durable Run/Step/Attempt and held G20.3 Runner RPC/isolation source foundation, plus future Capability Brokers, Prepare/Commit, depth-1 Child Agents and legacy cutover contract.
- [`schemas/`](./schemas/) — strict Draft 2020-12 Agent Runtime Manifest, Capability Grant, Runner RPC, and Run Event schemas; positive/negative fixtures live under [`fixtures/agent-runtime/`](./fixtures/agent-runtime/).
...
- [`memory-benchmark-workflow.md`](./memory-benchmark-workflow.md) — Memory v2 synthetic Golden/regression lifecycle, native v1-versus-v2 isolated reader capture, strict observations, deterministic scorer, immutable reports, and non-promotional boundary.
