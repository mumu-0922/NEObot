...
- [`agent-runtime.md`](./agent-runtime.md) — current Hermes-style `local_direct` Skill execution plus the retained optional G20/G21 isolated OCI Runtime contracts.
...
- [`schemas/`](./schemas/) — strict Draft 2020-12 Agent Runtime Manifest, Capability Grant, Runner RPC, Run Event, Cron revision, Skill Draft, operations-policy, production-closure, staged control/Root-canary activation, immutable Root-canary plan, and exact-host bundle schemas; positive/negative fixtures live under [`fixtures/agent-runtime/`](./fixtures/agent-runtime/).
