...
import { getWebSearchDegradationStatus } from "@/lib/search/fusion";
...
const SourceFusionNotice: React.FC<SourceFusionNoticeProps> = ({
  ...
  const status = getWebSearchDegradationStatus(metadata);
  ...
  return (
    ...
    >
      ...
      <span>
        {t(
          status === "partial"
            ? "webSearchPartiallyUnavailable"
            : "webSearchTemporarilyUnavailable",
        )}
      </span>
    </div>
  );
