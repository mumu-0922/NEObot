...
import { BookText, ChevronDown, Library, ShieldAlert } from "lucide-react";
...
const KnowledgeEvidenceBlock: React.FC<KnowledgeEvidenceBlockProps> = ({
  ...
  const showNoEvidence = knowledge.evidenceUsed === false || !hasCitations;
  if (!hasCitations && !showNoEvidence) return null;

  const heading = hasCitations
    ? t("citationsHeading", { count: citations.length })
    : t("knowledgeStatusHeading");
  const statusText = hasCitations
    ? t("verifiedEvidenceUsed")
    : knowledge.mode === "strict"
      ? t("strictKnowledgeRefused", {
          outcome: knowledge.outcome || "unknown",
        })
      : t("noKnowledgeEvidenceUsed");
