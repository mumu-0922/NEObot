...
type ServerGenerationStatus =
  "idle" | "pending" | "streaming" | "completed" | "failed" | "cancelled";

interface ServerGenerationError {
  code?: string;
  message: string;
  recoverable?: boolean;
  requestId?: string;
...
const toStoreSessionFromServer = (session: ChatCrudSession): Session =>
  normalizeSession({
    id: session.id,
    title: session.title,
...
const getServerGenerationError = (error: unknown): ServerGenerationError => {
  if (error instanceof Error) {
    return { message: error.message };
  }

  return { message: "Server stream failed." };
};

const getTerminalServerGenerationStatus = (
  result: ChatStreamRunResult,
): Exclude<ServerGenerationStatus, "idle" | "pending" | "streaming"> => {
...
export const useChatStore = create<ChatState>()(
  persist(
    (set, get) => ({
      ...
      serverReadState: createEmptyServerReadState(),
      selectedModel: "",
      ...
      refreshServerSessions: async () => {
        ...
        try {
          ...
          if (nextSessionId) {
            return get().selectServerSession(nextSessionId);
          }
