...
wrapper_source="${script_dir}/run-memory-single-user-bounded-miss-validation-from-vault.sh"
runner_source="${script_dir}/run-memory-regression.sh"
live_gate_source="${script_dir}/../backend/internal/memorycapture/live_gate.go"
validation_approval="I_UNDERSTAND_THIS_USES_REAL_FROZEN_MEMORY_SINGLE_USER_BOUNDED_MISS_VALIDATION_QUOTA"
for source in "${wrapper_source}" "${runner_source}" "${live_gate_source}"; do
  if [[ ! -f "${source}" ]] || ! grep -Fq -- "${validation_approval}" "${source}"; then
    echo "Memory single-user bounded-miss Validation Vault test: approval literal drifted across wrapper, runner, or Go live gate" >&2
    exit 1
  fi
done
temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/mm-chat-memory-single-user-bounded-miss-vault-test.XXXXXX")"
trap 'find "${temp_dir}" -depth -delete' EXIT
chmod 700 "${temp_dir}"

project_dir="${temp_dir}/project"
mkdir -p "${project_dir}/scripts" "${project_dir}/data/memory-benchmark/v2-regression"
cp "${wrapper_source}" "${project_dir}/scripts/run-memory-single-user-bounded-miss-validation-from-vault.sh"
chmod 755 "${project_dir}/scripts/run-memory-single-user-bounded-miss-validation-from-vault.sh"
...
model=""
live_approval=""
single_user_bounded_miss_approval=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --single-user-bounded-miss-validation-approval) single_user_bounded_miss_approval="$2"; shift 2 ;;
...
if [[ "${capture_mode}" != "production_fixed_memory_judge_negative_guard_double_confirmation_single_user_bounded_miss_validation" ||
  ...
  "${single_user_bounded_miss_approval}" != "I_UNDERSTAND_THIS_USES_REAL_FROZEN_MEMORY_SINGLE_USER_BOUNDED_MISS_VALIDATION_QUOTA" ]]; then
  echo "fake runner: fixed single-user bounded-miss Validation tuple drifted" >&2
  exit 92
fi
...
if [[ -n "${FAKE_RUNNER_ARTIFACT:-}" ]]; then
  printf 'private-aggregate-validation-test-artifact\n' >"${output}/fixed-memory-judge-negative-guard-double-confirmation-single-user-bounded-miss-production-validation.json"
  chmod 600 "${output}/fixed-memory-judge-negative-guard-double-confirmation-single-user-bounded-miss-production-validation.json"
fi
...
if [[ "${1:-}" == "image" && "${2:-}" == "inspect" ]]; then
  printf '%s\n' 'sha256:8a4f047e0a7776088037059ab0d956e3dfffad6c81de99e2260b0ec36128900f'
  exit 0
fi
...
cost_basis="${temp_dir}/cost-v25-single-user-bounded-miss-validation.json"
printf '{"schemaVersion":"neo-chat.memory-regression-cost-basis.v25-single-user-bounded-miss-validation.v1"}\n' >"${cost_basis}"
chmod 600 "${cost_basis}"
wrapper="${project_dir}/scripts/run-memory-single-user-bounded-miss-validation-from-vault.sh"
...
assert_no_export_directory() {
  if find "${private_tmp}" -mindepth 1 -maxdepth 1 -name 'mm-chat-memory-single-user-bounded-miss-credentials.*' -print -quit | grep -q .; then
    echo "Memory single-user bounded-miss Validation Vault test: exported credential directory remains" >&2
    exit 1
  fi
}
...
run_status_case() {
  ...
  if [[ ${status} -ne ${expected_status} ]]; then
    echo "Memory single-user bounded-miss Validation Vault test: ${name} status ${status}, want ${expected_status}" >&2
    cat "${stdout}" "${stderr}" >&2
    exit 1
  fi
  if [[ "$(cat "${runner_log}")" != "runner-started" ]]; then
    echo "Memory single-user bounded-miss Validation Vault test: ${name} runner did not execute exactly once" >&2
    exit 1
  fi
  if grep -Fq 'test-bge-secret' "${stdout}" "${stderr}" ||
    grep -Fq 'test-luna-secret' "${stdout}" "${stderr}"; then
    echo "Memory single-user bounded-miss Validation Vault test: ${name} leaked a credential" >&2
    exit 1
  fi
  if grep -Fq ' build ' "${docker_log}" ||
    ...
    echo "Memory single-user bounded-miss Validation Vault test: ${name} mutated or pulled the active backend image" >&2
    exit 1
  fi
  if [[ "${artifact}" == "yes" && ! -f "${output}/fixed-memory-judge-negative-guard-double-confirmation-single-user-bounded-miss-production-validation.json" ]]; then
    echo "Memory single-user bounded-miss Validation Vault test: ${name} lost retained aggregate artifact" >&2
    exit 1
  fi
  assert_no_export_directory
}
...
if [[ ${partial_status} -eq 0 || -s "${runner_log}" ]]; then
  echo "Memory single-user bounded-miss Validation Vault test: partial export did not fail before Validation" >&2
  exit 1
fi
assert_no_export_directory
...
for signal_name in INT TERM HUP; do
  ...
  if [[ ! -f "${signal_marker}" ]]; then
    echo "Memory single-user bounded-miss Validation Vault test: ${signal_name} runner did not start" >&2
  ...
  if [[ ${signal_status} -eq 0 ]]; then
    echo "Memory single-user bounded-miss Validation Vault test: ${signal_name} returned success" >&2
    exit 1
  fi
  assert_no_export_directory
done
...
if [[ ${denied_status} -eq 0 || -s "${docker_log}" ]]; then
  echo "Memory single-user bounded-miss Validation Vault test: missing approvals reached Docker" >&2
  exit 1
fi
assert_no_export_directory

bash -n "${wrapper_source}"
echo "Memory single-user bounded-miss Validation Vault lifecycle: passed"
