...
wrapper_source="${script_dir}/run-memory-abstention-confirmation-validation-from-vault.sh"
runner_source="${script_dir}/run-memory-regression.sh"
live_gate_source="${script_dir}/../backend/internal/memorycapture/live_gate.go"
validation_approval="I_UNDERSTAND_THIS_USES_REAL_FROZEN_MEMORY_ABSTENTION_CONFIRMATION_VALIDATION_QUOTA"
for source in "${wrapper_source}" "${runner_source}" "${live_gate_source}"; do
  if [[ ! -f "${source}" ]] || ! grep -Fq -- "${validation_approval}" "${source}"; then
    echo "Memory abstention-confirmation Validation Vault test: approval literal drifted across wrapper, runner, or Go live gate" >&2
    exit 1
  fi
done
temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/mm-chat-memory-abstention-confirmation-vault-test.XXXXXX")"
trap 'find "${temp_dir}" -depth -delete' EXIT
chmod 700 "${temp_dir}"

project_dir="${temp_dir}/project"
mkdir -p "${project_dir}/scripts" "${project_dir}/data/memory-benchmark/v2-regression"
cp "${wrapper_source}" "${project_dir}/scripts/run-memory-abstention-confirmation-validation-from-vault.sh"
chmod 755 "${project_dir}/scripts/run-memory-abstention-confirmation-validation-from-vault.sh"
...
model=""
live_approval=""
abstention_confirmation_approval=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --abstention-confirmation-validation-approval) abstention_confirmation_approval="$2"; shift 2 ;;
...
if [[ "${capture_mode}" != "production_fixed_memory_judge_negative_guard_abstention_confirmation_validation" ||
  ...
  "${abstention_confirmation_approval}" != "I_UNDERSTAND_THIS_USES_REAL_FROZEN_MEMORY_ABSTENTION_CONFIRMATION_VALIDATION_QUOTA" ]]; then
  echo "fake runner: fixed abstention-confirmation Validation tuple drifted" >&2
  exit 92
fi
...
if [[ -n "${FAKE_RUNNER_ARTIFACT:-}" ]]; then
  printf 'private-aggregate-validation-test-artifact\n' >"${output}/fixed-memory-judge-negative-guard-abstention-confirmation-production-validation.json"
  chmod 600 "${output}/fixed-memory-judge-negative-guard-abstention-confirmation-production-validation.json"
fi
...
cost_basis="${temp_dir}/cost-v21.json"
printf '{"schemaVersion":"neo-chat.memory-regression-cost-basis.v21"}\n' >"${cost_basis}"
chmod 600 "${cost_basis}"
wrapper="${project_dir}/scripts/run-memory-abstention-confirmation-validation-from-vault.sh"
...
assert_no_export_directory() {
  if find "${private_tmp}" -mindepth 1 -maxdepth 1 -name 'mm-chat-memory-abstention-confirmation-credentials.*' -print -quit | grep -q .; then
    echo "Memory abstention-confirmation Validation Vault test: exported credential directory remains" >&2
    exit 1
  fi
}
...
run_status_case() {
  ...
  if [[ ${status} -ne ${expected_status} ]]; then
    echo "Memory abstention-confirmation Validation Vault test: ${name} status ${status}, want ${expected_status}" >&2
    cat "${stdout}" "${stderr}" >&2
    exit 1
  fi
  if [[ "$(cat "${runner_log}")" != "runner-started" ]]; then
    echo "Memory abstention-confirmation Validation Vault test: ${name} runner did not execute exactly once" >&2
    exit 1
  fi
  if grep -Fq 'test-bge-secret' "${stdout}" "${stderr}" ||
    grep -Fq 'test-luna-secret' "${stdout}" "${stderr}"; then
    echo "Memory abstention-confirmation Validation Vault test: ${name} leaked a credential" >&2
    exit 1
  fi
  if grep -Fq ' build ' "${docker_log}" ||
    ...
    echo "Memory abstention-confirmation Validation Vault test: ${name} mutated or pulled the active backend image" >&2
    exit 1
  fi
  if [[ "${artifact}" == "yes" && ! -f "${output}/fixed-memory-judge-negative-guard-abstention-confirmation-production-validation.json" ]]; then
    echo "Memory abstention-confirmation Validation Vault test: ${name} lost retained aggregate artifact" >&2
    exit 1
  fi
  assert_no_export_directory
}
...
if [[ ${partial_status} -eq 0 || -s "${runner_log}" ]]; then
  echo "Memory abstention-confirmation Validation Vault test: partial export did not fail before Validation" >&2
  exit 1
fi
assert_no_export_directory
...
for signal_name in INT TERM HUP; do
  ...
  if [[ ! -f "${signal_marker}" ]]; then
    echo "Memory abstention-confirmation Validation Vault test: ${signal_name} runner did not start" >&2
  ...
  if [[ ${signal_status} -eq 0 ]]; then
    echo "Memory abstention-confirmation Validation Vault test: ${signal_name} returned success" >&2
    exit 1
  fi
  assert_no_export_directory
done
...
if [[ ${denied_status} -eq 0 || -s "${docker_log}" ]]; then
  echo "Memory abstention-confirmation Validation Vault test: missing approvals reached Docker" >&2
  exit 1
fi
assert_no_export_directory

bash -n "${wrapper_source}"
echo "Memory abstention-confirmation Validation Vault lifecycle: passed"
