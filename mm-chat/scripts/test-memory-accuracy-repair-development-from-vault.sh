...
wrapper_source="${script_dir}/run-memory-accuracy-repair-development-from-vault.sh"
temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/mm-chat-memory-accuracy-repair-vault-test.XXXXXX")"
trap 'find "${temp_dir}" -depth -delete' EXIT
chmod 700 "${temp_dir}"

project_dir="${temp_dir}/project"
mkdir -p "${project_dir}/scripts" "${project_dir}/data/memory-benchmark/v2-regression"
cp "${wrapper_source}" "${project_dir}/scripts/run-memory-accuracy-repair-development-from-vault.sh"
chmod 755 "${project_dir}/scripts/run-memory-accuracy-repair-development-from-vault.sh"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --accuracy-repair-memory-judge-approval) accuracy_repair_approval="$2"; shift 2 ;;
...
if [[ "${capture_mode}" != "development_fixed_memory_judge_negative_guard_buffered_accuracy_repair" ||
  ...
  "${accuracy_repair_approval}" != "I_UNDERSTAND_THIS_USES_REAL_MEMORY_ACCURACY_REPAIR_QUOTA" ]]; then
  echo "fake runner: fixed accuracy-repair Development tuple drifted" >&2
  exit 92
fi
...
if [[ -n "${FAKE_RUNNER_ARTIFACT:-}" ]]; then
  printf 'private-aggregate-development-test-artifact\n' >"${output}/fixed-memory-judge-negative-guard-buffered-accuracy-repair-development.json"
  chmod 600 "${output}/fixed-memory-judge-negative-guard-buffered-accuracy-repair-development.json"
fi
...
printf '{"schemaVersion":"neo-chat.memory-regression-cost-basis.v15"}\n' >"${cost_basis}"
chmod 600 "${cost_basis}"
wrapper="${project_dir}/scripts/run-memory-accuracy-repair-development-from-vault.sh"
...
assert_no_export_directory() {
  if find "${private_tmp}" -mindepth 1 -maxdepth 1 -name 'mm-chat-memory-accuracy-repair-credentials.*' -print -quit | grep -q .; then
    echo "Memory accuracy-repair Development Vault test: exported credential directory remains" >&2
    exit 1
  fi
}
...
run_status_case() {
  ...
  if [[ ${status} -ne ${expected_status} ]]; then
    echo "Memory accuracy-repair Development Vault test: ${name} status ${status}, want ${expected_status}" >&2
    cat "${stdout}" "${stderr}" >&2
    exit 1
  fi
  if [[ "$(cat "${runner_log}")" != "runner-started" ]]; then
    echo "Memory accuracy-repair Development Vault test: ${name} runner did not execute exactly once" >&2
    exit 1
  fi
  if grep -Fq 'test-bge-secret' "${stdout}" "${stderr}" ||
    grep -Fq 'test-luna-secret' "${stdout}" "${stderr}"; then
    echo "Memory accuracy-repair Development Vault test: ${name} leaked a credential" >&2
    exit 1
  fi
  if grep -Fq ' build ' "${docker_log}" ||
    ...
    echo "Memory accuracy-repair Development Vault test: ${name} mutated or pulled the active backend image" >&2
    exit 1
  fi
  if [[ "${artifact}" == "yes" && ! -f "${output}/fixed-memory-judge-negative-guard-buffered-accuracy-repair-development.json" ]]; then
    echo "Memory accuracy-repair Development Vault test: ${name} lost retained aggregate artifact" >&2
    exit 1
  fi
  assert_no_export_directory
}
...
if [[ ${partial_status} -eq 0 || -s "${runner_log}" ]]; then
  echo "Memory accuracy-repair Development Vault test: partial export did not fail before Development" >&2
  exit 1
fi
assert_no_export_directory
...
for signal_name in INT TERM HUP; do
  ...
  if [[ ! -f "${signal_marker}" ]]; then
    echo "Memory accuracy-repair Development Vault test: ${signal_name} runner did not start" >&2
  ...
  if [[ ${signal_status} -eq 0 ]]; then
    echo "Memory accuracy-repair Development Vault test: ${signal_name} returned success" >&2
    exit 1
  fi
  assert_no_export_directory
done
...
if [[ ${denied_status} -eq 0 || -s "${docker_log}" ]]; then
  echo "Memory accuracy-repair Development Vault test: missing approvals reached Docker" >&2
  exit 1
fi
assert_no_export_directory

bash -n "${wrapper_source}"
echo "Memory accuracy-repair Development Vault lifecycle: passed"
