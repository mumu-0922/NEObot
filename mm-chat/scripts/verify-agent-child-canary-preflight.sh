...
for command in bash date docker jq openssl python3 sed sha256sum; do
  command -v "${command}" >/dev/null 2>&1 || { echo "G21.4 preflight: ${command} is required" >&2; exit 1; }
done
...
make_certificate child-client spiffe://neo-chat/agent-runtime-child-canary
make_certificate child-server-ca neo-runner-child-test-ca
...
cp "${project_dir}/docs/contracts/fixtures/agent-runtime/neo-agent-child-run-canary-plan.valid.json" \
  "${work_dir}/child-plan.json"
...
sed "s|${zero}|${nonzero}|g" "${project_dir}/config/agent-runner/release-manifest.example.json" |
  jq '.approved=true | .runnerVersion="g21.4-preflight-test"' >"${work_dir}/release-manifest.json"
cp "${work_dir}/release-manifest.json" "${work_dir}/child-release-manifest.json"
chmod 600 "${work_dir}/release-manifest.json"
chmod 600 "${work_dir}/child-release-manifest.json"
...
for prefix in ("root", "broker", "project", "child"):
...
approval_payload = {
    ...
    "decision": "approved",
    "release": {"gitCommit": commit, "migrationHead": 93},
...
ready("child-run-canary-activation", "child-client.crt", "child-server-ca.crt", {
    "canaryPlanSha256": fp(work / "child-plan.json"),
    "authorityPublicKeySha256": fp(work / "child-authority-public-key"),
})
PY
...
sed \
  ...
  -e 's|change-me-agent-child-canary-postgres|test-agent-child-canary-password|g' \
  ...
  -e 's|^AGENT_CHILD_CANARY_RUNNER_URL=.*|AGENT_CHILD_CANARY_RUNNER_URL=https://10.0.0.8:9443/internal/neo-runner/v1/rpc|' \
  ...
  -e "s|^AGENT_CHILD_CANARY_CLIENT_CERT_SOURCE=.*|AGENT_CHILD_CANARY_CLIENT_CERT_SOURCE=${work_dir}/child-client.crt|" \
  -e "s|^AGENT_CHILD_CANARY_CLIENT_KEY_SOURCE=.*|AGENT_CHILD_CANARY_CLIENT_KEY_SOURCE=${work_dir}/child-client.key|" \
  -e "s|^AGENT_CHILD_CANARY_SERVER_CA_SOURCE=.*|AGENT_CHILD_CANARY_SERVER_CA_SOURCE=${work_dir}/child-server-ca.crt|" \
  -e "s|^AGENT_CHILD_CANARY_RELEASE_MANIFEST_SOURCE=.*|AGENT_CHILD_CANARY_RELEASE_MANIFEST_SOURCE=${work_dir}/child-release-manifest.json|" \
  -e "s|^AGENT_CHILD_CANARY_PRODUCTION_POLICY_SOURCE=.*|AGENT_CHILD_CANARY_PRODUCTION_POLICY_SOURCE=${project_dir}/config/agent-runner/production-policy.json|" \
  -e "s|^AGENT_CHILD_CANARY_ACTIVATION_SOURCE=.*|AGENT_CHILD_CANARY_ACTIVATION_SOURCE=${work_dir}/child-run-canary-activation-ready.json|" \
  -e "s|^AGENT_CHILD_CANARY_PLAN_SOURCE=.*|AGENT_CHILD_CANARY_PLAN_SOURCE=${work_dir}/child-plan.json|" \
  -e "s|^AGENT_CHILD_CANARY_AUTHORITY_PRIVATE_KEY_SOURCE=.*|AGENT_CHILD_CANARY_AUTHORITY_PRIVATE_KEY_SOURCE=${work_dir}/child-authority-private-key|" \
  -e "s|^AGENT_CHILD_CANARY_AUTHORITY_PUBLIC_KEY_SOURCE=.*|AGENT_CHILD_CANARY_AUTHORITY_PUBLIC_KEY_SOURCE=${work_dir}/child-authority-public-key|" \
  -e 's|^AGENT_CHILD_CANARY_RELEASE_GIT_COMMIT=.*|AGENT_CHILD_CANARY_RELEASE_GIT_COMMIT=1111111111111111111111111111111111111111|' \
...
assert_rejected(){
  ...
  if output="$(bash "${preflight}" "${env_file}" 2>&1)"; then
    echo "G21.4 preflight: accepted ${name}" >&2
    exit 1
  fi
  [[ "${output}" == *"${expected}"* ]] || { echo "G21.4 preflight: unexpected ${name} rejection" >&2; exit 1; }
}
...
sed 's|^AGENT_PROJECT_MUTATION_CANARY_ENABLED=true$|AGENT_PROJECT_MUTATION_CANARY_ENABLED=false|' \
  "${valid_env}" >"${work_dir}/child-missing-prerequisite.env"
sed 's|^AGENT_DELEGATION_ENABLED=true$|AGENT_DELEGATION_ENABLED=false|' \
  "${valid_env}" >"${work_dir}/child-delegation-disabled.env"
sed 's|^AGENT_CHILD_CANARY_RUNNER_URL=.*|AGENT_CHILD_CANARY_RUNNER_URL=https://runner.internal:9443/internal/neo-runner/v1/rpc|' \
  "${valid_env}" >"${work_dir}/child-runner-hostname.env"
sed 's|^AGENT_CHILD_CANARY_CLIENT_IDENTITY=.*|AGENT_CHILD_CANARY_CLIENT_IDENTITY=spiffe://neo-chat/agent-runtime-root-canary|' \
  "${valid_env}" >"${work_dir}/child-reused-identity.env"
sed 's|postgres://agent_child_canary_app:test-agent-child-canary-password@|postgres://agent_project_canary_app:test-agent-child-canary-password@|' \
  "${valid_env}" >"${work_dir}/child-shared-principal.env"
sed "s|^AGENT_CHILD_CANARY_AUTHORITY_PUBLIC_KEY_SOURCE=.*|AGENT_CHILD_CANARY_AUTHORITY_PUBLIC_KEY_SOURCE=${work_dir}/root-authority-public-key|" \
  "${valid_env}" >"${work_dir}/child-reused-key.env"
jq '.childBudget.maxToolCalls=.parentBudget.maxToolCalls' \
  "${work_dir}/child-plan.json" >"${work_dir}/widened-child-plan.json"
chmod 600 "${work_dir}/widened-child-plan.json"
sed "s|^AGENT_CHILD_CANARY_PLAN_SOURCE=.*|AGENT_CHILD_CANARY_PLAN_SOURCE=${work_dir}/widened-child-plan.json|" \
  "${valid_env}" >"${work_dir}/widened-child-plan.env"
cp "${project_dir}/docs/contracts/fixtures/agent-runtime/neo-agent-child-run-canary-activation.valid.json" \
  "${work_dir}/child-held-activation.json"
chmod 600 "${work_dir}/child-held-activation.json"
sed "s|^AGENT_CHILD_CANARY_ACTIVATION_SOURCE=.*|AGENT_CHILD_CANARY_ACTIVATION_SOURCE=${work_dir}/child-held-activation.json|" \
  "${valid_env}" >"${work_dir}/child-held-activation.env"
cp "${valid_env}" "${work_dir}/child-credential.env"
printf '%s\n' 'AGENT_CHILD_CANARY_PROVIDER_API_KEY=forbidden-test-value' >>"${work_dir}/child-credential.env"
chmod 600 "${work_dir}"/*.env

assert_rejected child-missing-prerequisite 'requires G21.0-G21.3 enabled'
assert_rejected child-delegation-disabled 'must match the dedicated G21.4 Child canary flag'
assert_rejected child-runner-hostname 'exact private HTTPS Runner RPC URL'
assert_rejected child-reused-identity 'identities must be exact and distinct'
assert_rejected child-shared-principal 'must use distinct database principals'
assert_rejected child-reused-key 'must not reuse prior activation, authority or TLS material'
assert_rejected widened-child-plan 'strict one-Parent/one-Child plan'
assert_rejected child-held-activation 'is not READY for G21.4'
assert_rejected child-credential 'must not receive Broker, relay, Egress, Secret, MCP, Provider, vault or Redis configuration'

compose_json="${work_dir}/compose.json"
docker compose --env-file "${valid_env}" -f "${project_dir}/compose.yml" \
  --profile agent-runtime-child-canary config --format json >"${compose_json}"
python3 - "${compose_json}" <<'PY'
import json,sys
service=json.load(open(sys.argv[1]))["services"]["agent-runtime-child-canary"]
...
assert all(volume.get("bind", {}).get("create_host_path") is False for volume in service["volumes"])
assert set(service["networks"]) == {"private"}
assert not service.get("ports") and not service.get("secrets")
environment=service["environment"]
for marker in ("S3_", "MCP_", "PROVIDER_", "VAULT_", "REDIS_", "RELAY_", "SECRET"):
    assert not any(marker in key for key in environment), (marker,environment)
for key in (
    "AGENT_RUNTIME_ENABLED", "AGENT_BROKER_READ_ONLY_ENABLED", "AGENT_BROKER_MUTATION_ENABLED",
    "AGENT_SCHEDULER_ENABLED", "AGENT_SKILL_INSTALL_ENABLED", "AGENT_LEARNING_ENABLED",
):
    assert environment[key] == "false"
for key in (
    "AGENT_CHILD_CANARY_ENABLED", "AGENT_DELEGATION_ENABLED", "AGENT_RUNNER_CONTROL_ENABLED",
    "AGENT_ROOT_RUN_CANARY_ENABLED", "AGENT_BROKER_ARTIFACT_CANARY_ENABLED",
    "AGENT_PROJECT_MUTATION_CANARY_ENABLED",
):
    assert environment[key] == "true"
PY

printf '%s\n' 'Agent Runtime G21.4 enabled preflight verification: passed.'
