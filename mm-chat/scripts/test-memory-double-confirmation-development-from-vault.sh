...
wrapper_source="${script_dir}/run-memory-double-confirmation-development-from-vault.sh"
...
cp "${wrapper_source}" "${project_dir}/scripts/run-memory-double-confirmation-development-from-vault.sh"
chmod 755 "${project_dir}/scripts/run-memory-double-confirmation-development-from-vault.sh"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --double-confirmation-development-approval) double_confirmation_approval="$2"; shift 2 ;;
...
if [[ "${capture_mode}" != "development_fixed_memory_judge_negative_guard_double_confirmation" ||
  ...
  "${double_confirmation_approval}" != "I_UNDERSTAND_THIS_USES_REAL_MEMORY_DOUBLE_CONFIRMATION_DEVELOPMENT_QUOTA" ]]; then
  echo "fake runner: fixed double-confirmation Development tuple drifted" >&2
  exit 92
fi
...
if [[ -n "${FAKE_RUNNER_ARTIFACT:-}" ]]; then
  printf 'private-aggregate-development-test-artifact\n' >"${output}/fixed-memory-judge-negative-guard-double-confirmation-development.json"
  chmod 600 "${output}/fixed-memory-judge-negative-guard-double-confirmation-development.json"
fi
...
cost_basis="${temp_dir}/cost-v22.json"
printf '{"schemaVersion":"neo-chat.memory-regression-cost-basis.v22-double-confirmation-development.v1"}\n' >"${cost_basis}"
chmod 600 "${cost_basis}"
wrapper="${project_dir}/scripts/run-memory-double-confirmation-development-from-vault.sh"
...
assert_no_export_directory() {
  if find "${private_tmp}" -mindepth 1 -maxdepth 1 -name 'mm-chat-memory-double-confirmation-credentials.*' -print -quit | grep -q .; then
    echo "Memory double-confirmation Development Vault test: exported credential directory remains" >&2
    exit 1
  fi
}
...
run_status_case() {
  ...
  if [[ ${status} -ne ${expected_status} ]]; then
    echo "Memory double-confirmation Development Vault test: ${name} status ${status}, want ${expected_status}" >&2
    cat "${stdout}" "${stderr}" >&2
    exit 1
  fi
  if [[ "$(cat "${runner_log}")" != "runner-started" ]]; then
    echo "Memory double-confirmation Development Vault test: ${name} runner did not execute exactly once" >&2
    exit 1
  fi
  if grep -Fq 'test-bge-secret' "${stdout}" "${stderr}" ||
    grep -Fq 'test-luna-secret' "${stdout}" "${stderr}"; then
    echo "Memory double-confirmation Development Vault test: ${name} leaked a credential" >&2
    exit 1
  fi
  if grep -Fq ' build ' "${docker_log}" ||
    ...
    echo "Memory double-confirmation Development Vault test: ${name} mutated or pulled the active backend image" >&2
    exit 1
  fi
  if [[ "${artifact}" == "yes" && ! -f "${output}/fixed-memory-judge-negative-guard-double-confirmation-development.json" ]]; then
    echo "Memory double-confirmation Development Vault test: ${name} lost retained aggregate artifact" >&2
    exit 1
  fi
  assert_no_export_directory
}
...
if [[ ${partial_status} -eq 0 || -s "${runner_log}" ]]; then
  echo "Memory double-confirmation Development Vault test: partial export did not fail before Development" >&2
  exit 1
fi
assert_no_export_directory
...
for signal_name in INT TERM HUP; do
  ...
  if [[ ! -f "${signal_marker}" ]]; then
    echo "Memory double-confirmation Development Vault test: ${signal_name} runner did not start" >&2
  ...
  if [[ ${signal_status} -eq 0 ]]; then
    echo "Memory double-confirmation Development Vault test: ${signal_name} returned success" >&2
    exit 1
  fi
  assert_no_export_directory
done
...
if [[ ${denied_status} -eq 0 || -s "${docker_log}" ]]; then
  echo "Memory double-confirmation Development Vault test: missing approvals reached Docker" >&2
  exit 1
fi
assert_no_export_directory

bash -n "${wrapper_source}"
echo "Memory double-confirmation Development Vault lifecycle: passed"
