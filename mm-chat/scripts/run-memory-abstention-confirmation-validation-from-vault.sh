...
usage() {
  cat <<'EOF'
usage: run-memory-abstention-confirmation-validation-from-vault.sh \
  --cost-basis <schema-v21-validation-json> \
...
the separately versioned 100-case abstention-confirmation Validation lane, and wipe the exported
...
abstention_confirmation_validation_approval_set=false
env_file_set=false
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --abstention-confirmation-validation-approval)
      [[ $# -ge 2 && "${abstention_confirmation_validation_approval_set}" == false ]] || { usage >&2; exit 2; }
      abstention_confirmation_validation_approval="$2"
      abstention_confirmation_validation_approval_set=true
      shift 2
      ;;
    --env-file)
...
if [[ "${cost_basis_set}" != true || "${output_parent_set}" != true ||
  ...
  "${abstention_confirmation_validation_approval}" != "I_UNDERSTAND_THIS_USES_REAL_FROZEN_MEMORY_ABSTENTION_CONFIRMATION_VALIDATION_QUOTA" ]]; then
  echo "Memory abstention-confirmation Validation: exact one-run approvals are required" >&2
  exit 2
fi

for required in "${compose_file}" "${runner_script}" "${env_file}" "${cost_basis}"; do
  if [[ ! -f "${required}" || -L "${required}" ]]; then
    echo "Memory abstention-confirmation Validation: required input is missing or symlinked" >&2
    exit 1
  fi
done
if [[ "$(stat -c '%a' "${env_file}")" != "600" ||
  "$(stat -c '%a' "${cost_basis}")" != "600" ]]; then
  echo "Memory abstention-confirmation Validation: env and cost-basis files must be mode 0600" >&2
  exit 1
fi
if [[ ! -d "${regression_root}" || -L "${regression_root}" ]]; then
  echo "Memory abstention-confirmation Validation: protected regression root is missing or symlinked" >&2
  exit 1
fi
for required in fixtures.json corpus.json audit.json manifest.json; do
  if [[ ! -f "${regression_root}/${required}" || -L "${regression_root}/${required}" ]]; then
    echo "Memory abstention-confirmation Validation: protected regression input is missing or symlinked" >&2
    exit 1
  fi
done
...
for part in raw.parts[1:]:
    ...
    if probe.exists() or probe.is_symlink():
        if probe.is_symlink():
            raise SystemExit("Memory abstention-confirmation Validation: output path contains a symlink")
        if probe == raw and not probe.is_dir():
            raise SystemExit("Memory abstention-confirmation Validation: output parent is not a directory")
...
case "${output_parent}" in
  ...
  "${project_dir}/backup"|"${project_dir}/backup/"*)
    echo "Memory abstention-confirmation Validation: output cannot enter protected runtime state" >&2
    exit 1
    ;;
esac
...
if [[ ! -d "${output_parent}" || ! -w "${output_parent}" || ! -x "${output_parent}" ||
  ...
  echo "Memory abstention-confirmation Validation: output parent must be a writable mode-0700 directory" >&2
  exit 1
fi
...
if ! "${docker_bin}" compose version >/dev/null 2>&1; then
  ...
  if [[ -x "${windows_docker}" ]] && "${windows_docker}" compose version >/dev/null 2>&1; then
    ...
    echo "Memory abstention-confirmation Validation: Docker Compose is required" >&2
    exit 1
  fi
fi
...
if [[ "${actual_admin_image_id}" != "${candidate_admin_image_id}" ]]; then
  echo "Memory abstention-confirmation Validation: pinned credential-export helper image is unavailable or drifted" >&2
  exit 1
fi
...
compose_run_help="$("${docker_bin}" compose run --help 2>&1)" || {
  echo "Memory abstention-confirmation Validation: cannot inspect Compose run capabilities" >&2
  exit 1
}
...
cleanup() {
  ...
  if [[ -e "${credential_dir}" ]]; then
    cleanup_failed=true
    echo "Memory abstention-confirmation Validation: exported credential cleanup failed" >&2
  fi
  if [[ "${cleanup_failed}" == true && ${status} -eq 0 ]]; then
    status=1
  fi
  if [[ "${cleanup_failed}" == false ]]; then
    echo "Memory abstention-confirmation Validation: one-run credential copies destroyed"
  fi
  exit "${status}"
}
...
if [[ ! -f "${bge_credential}" || -L "${bge_credential}" ||
  ...
  echo "Memory abstention-confirmation Validation: exported credential pair failed preflight" >&2
  exit 1
fi
...
bash "${runner_script}" \
  ...
  --abstention-confirmation-validation-approval "${abstention_confirmation_validation_approval}"
