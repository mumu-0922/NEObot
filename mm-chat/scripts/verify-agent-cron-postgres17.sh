...
container_name="neo-chat-agent-cron-pg17-$RANDOM-$$"
restore_container_name="${container_name}-restore"
database_name="neo_chat_agent_cron_drill"
database_user="postgres"
database_password="agent-cron-drill-$(openssl rand -hex 16)"
work_dir="$(mktemp -d)"
...
log() { printf 'Agent Cron PostgreSQL 17 drill: %s\n' "$*"; }
...
log "checking Cron least privilege"
psql_command "${container_name}" "
DO \$\$
BEGIN
  IF to_regclass('public.agent_cron_templates') IS NULL
     OR NOT has_table_privilege('agent_cron_control','agent_cron_templates','SELECT')
     OR has_table_privilege('agent_cron_control','agent_cron_templates','INSERT')
     OR has_table_privilege('agent_cron_control','agent_cron_templates','UPDATE')
     OR has_table_privilege('agent_cron_control','agent_cron_templates','DELETE')
     OR has_table_privilege('go_api_runtime','agent_cron_templates','SELECT')
     OR has_table_privilege('agent_orchestrator_runtime','agent_cron_templates','SELECT')
     OR has_table_privilege('agent_runner_control','agent_cron_templates','SELECT')
     OR has_table_privilege('agent_effect_control','agent_cron_templates','SELECT')
     OR has_table_privilege('agent_delegation_control','agent_cron_templates','SELECT')
     OR NOT has_function_privilege('agent_cron_control','agent_cron_create_revision(text,uuid,bigint,text,jsonb,timestamp with time zone,text,text,text,text)','EXECUTE')
     OR NOT has_function_privilege('agent_cron_control','agent_cron_enqueue_trigger(text,text,bigint,text,text,text,text,jsonb,jsonb,text[],text)','EXECUTE')
     OR NOT has_function_privilege('agent_cron_control','agent_cron_reconcile(timestamp with time zone,integer)','EXECUTE')
     OR NOT has_function_privilege('agent_cron_control','agent_cron_prune(timestamp with time zone,integer)','EXECUTE')
     OR has_function_privilege('go_api_runtime','agent_cron_enqueue_trigger(text,text,bigint,text,text,text,text,jsonb,jsonb,text[],text)','EXECUTE')
     OR has_function_privilege('agent_orchestrator_runtime','agent_cron_claim_due(text,timestamp with time zone,integer,integer)','EXECUTE')
     OR has_function_privilege('agent_runner_control','agent_cron_claim_triggers(text,timestamp with time zone,integer,integer)','EXECUTE')
     OR has_function_privilege('agent_effect_control','agent_cron_reconcile(timestamp with time zone,integer)','EXECUTE')
     OR has_function_privilege('agent_delegation_control','agent_cron_prune(timestamp with time zone,integer)','EXECUTE')
     OR pg_has_role('agent_cron_control','agent_cron_owner','MEMBER') THEN
    RAISE EXCEPTION 'Agent Cron grants violate least privilege';
  END IF;
END
\$\$;" >/dev/null

log "running schedule, claim, restart, idempotency, overlap, and authority matrix"
(cd "${backend_dir}" && MM_CHAT_TEST_DATABASE_URL="${database_url}" \
  go test -count=1 -race -run '^TestPostgresAgentCron' ./internal/agentcron)
...
guard_status=$?
set -e
if [[ "${guard_status}" -eq 0 ]] || ! grep -Fq 'AGENT_CRON_DOWN_DATA_EXISTS' "${work_dir}/guard.log"; then
  cat "${work_dir}/guard.log" >&2
  exit 1
fi

log "dumping and restoring Cron authority"
...
SELECT concat_ws(',',
  ...
  (SELECT count(*) FROM agent_cron_revisions),
  (SELECT count(*) FROM agent_cron_approvals),
  (SELECT count(*) FROM agent_cron_approval_revocations),
  (SELECT count(*) FROM agent_cron_triggers),
  (SELECT count(*) FROM agent_cron_audit_events),
  (SELECT count(*) FROM agent_runs WHERE idempotency_key LIKE 'cron:%'));")"
...
if [[ "${source_counts}" != "${restore_counts}" ]]; then
  printf 'Cron dump/restore count mismatch: source=%s restore=%s\n' \
    "${source_counts}" "${restore_counts}" >&2
  exit 1
fi

log "proving clean 087 -> 088 -> 087 -> 094"
