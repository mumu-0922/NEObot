...
wrapper_source="${script_dir}/run-memory-negative-guard-development-from-vault.sh"
temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/mm-chat-memory-negative-guard-vault-test.XXXXXX")"
trap 'find "${temp_dir}" -depth -delete' EXIT
chmod 700 "${temp_dir}"

project_dir="${temp_dir}/project"
mkdir -p "${project_dir}/scripts" "${project_dir}/data/memory-benchmark/v2-regression"
cp "${wrapper_source}" "${project_dir}/scripts/run-memory-negative-guard-development-from-vault.sh"
chmod 755 "${project_dir}/scripts/run-memory-negative-guard-development-from-vault.sh"
...
if [[ "${capture_mode}" != "development_fixed_memory_judge_negative_guard" ||
  ...
  echo "fake runner: fixed negative-guard Development tuple drifted" >&2
  exit 92
fi
...
cost_basis="${temp_dir}/cost-v11.json"
printf '{"schemaVersion":"neo-chat.memory-regression-cost-basis.v11"}\n' >"${cost_basis}"
chmod 600 "${cost_basis}"
wrapper="${project_dir}/scripts/run-memory-negative-guard-development-from-vault.sh"
...
assert_no_export_directory() {
  if find "${private_tmp}" -mindepth 1 -maxdepth 1 -name 'mm-chat-memory-negative-guard-credentials.*' -print -quit | grep -q .; then
    echo "Memory negative-guard Development Vault test: exported credential directory remains" >&2
    exit 1
  fi
}
...
run_status_case() {
  ...
  if [[ ${status} -ne ${expected_status} ]]; then
    echo "Memory negative-guard Development Vault test: ${name} status ${status}, want ${expected_status}" >&2
    cat "${stdout}" "${stderr}" >&2
    exit 1
  fi
  if [[ "$(cat "${runner_log}")" != "runner-started" ]]; then
    echo "Memory negative-guard Development Vault test: ${name} runner did not execute exactly once" >&2
    exit 1
  fi
  if grep -Fq 'test-bge-secret' "${stdout}" "${stderr}" ||
    grep -Fq 'test-luna-secret' "${stdout}" "${stderr}"; then
    echo "Memory negative-guard Development Vault test: ${name} leaked a credential" >&2
    exit 1
  fi
  if [[ "${artifact}" == "yes" && ! -f "${output}/aggregate.json" ]]; then
    echo "Memory negative-guard Development Vault test: ${name} lost retained aggregate artifact" >&2
    exit 1
  fi
  assert_no_export_directory
}
...
if [[ ${partial_status} -eq 0 || -s "${runner_log}" ]]; then
  echo "Memory negative-guard Development Vault test: partial export did not fail before Development" >&2
  exit 1
fi
assert_no_export_directory
...
for signal_name in INT TERM HUP; do
  ...
  if [[ ! -f "${signal_marker}" ]]; then
    echo "Memory negative-guard Development Vault test: ${signal_name} runner did not start" >&2
  ...
  if [[ ${signal_status} -eq 0 ]]; then
    echo "Memory negative-guard Development Vault test: ${signal_name} returned success" >&2
    exit 1
  fi
  assert_no_export_directory
done
...
if [[ ${denied_status} -eq 0 || -s "${docker_log}" ]]; then
  echo "Memory negative-guard Development Vault test: missing approvals reached Docker" >&2
  exit 1
fi
assert_no_export_directory

bash -n "${wrapper_source}"
echo "Memory negative-guard Development Vault lifecycle: passed"
