...
func run(
	...
	case memorycapture.CaptureModeTransportStableMemoryJudge:
		configuredJudgeAuthority, err = buildConfiguredCandidateJudgeAuthority(options)
		if err != nil {
			return err
		}
		...
		if err := memorycapture.ValidateTransportStableMemoryJudgeCostAuthority(
			cost,
			configuredJudgeAuthority,
		); err != nil {
			return err
		}
		transportStableMemoryJudgeConfig, err =
			memorycapture.BuildTransportStableMemoryJudgeDevelopmentProfileConfig(
				protected,
				costHash,
				options.providerMode,
				configuredJudgeAuthority,
		...
		relevanceConfigurationHash, err =
			memorycapture.ConfigurationSHA256(transportStableMemoryJudgeConfig)
		if err != nil {
			return err
		}
		artifactNames = []string{
	...
	case memorycapture.CaptureModeBufferedMemoryJudge:
		configuredJudgeAuthority, err = buildConfiguredCandidateJudgeAuthority(options)
		if err != nil {
			return err
		}
		...
		if err := memorycapture.ValidateBufferedMemoryJudgeCostAuthority(
			cost,
			configuredJudgeAuthority,
		); err != nil {
			return err
		}
		bufferedMemoryJudgeConfig, err =
			memorycapture.BuildBufferedMemoryJudgeDevelopmentProfileConfig(
				protected,
				costHash,
				options.providerMode,
				configuredJudgeAuthority,
		...
		relevanceConfigurationHash, err =
			memorycapture.ConfigurationSHA256(bufferedMemoryJudgeConfig)
		if err != nil {
			return err
		}
		artifactNames = []string{
			memorycapture.BufferedMemoryJudgeArtifactName,
			"run-manifest.json",
		}
	...
	case memorycapture.CaptureModeProductionBufferedMemoryJudgeValidation:
		configuredJudgeAuthority, err = buildConfiguredCandidateJudgeAuthority(options)
		if err != nil {
			return err
		}
		...
		if err := memorycapture.ValidateProductionBufferedMemoryJudgeValidationCostAuthority(
			cost,
			configuredJudgeAuthority,
		); err != nil {
			return err
		}
		productionBufferedMemoryJudgeValidationConfig, err =
			memorycapture.BuildProductionBufferedMemoryJudgeValidationProfileConfig(
				protected,
				costHash,
				options.providerMode,
				configuredJudgeAuthority,
		...
		relevanceConfigurationHash, err =
			memorycapture.ConfigurationSHA256(productionBufferedMemoryJudgeValidationConfig)
		if err != nil {
			return err
		}
		artifactNames = []string{
	...
	if options.captureMode == memorycapture.CaptureModeBufferedMemoryJudge {
		return runBufferedMemoryJudgeDevelopment(
			ctx, stdout, options, startedAt, protected, cost, costHash,
			bufferedMemoryJudgeConfig, configuredJudgeAuthority,
			relevanceConfigurationHash, providers, adminDB, runtimeDB,
		)
	}
...
func runTransportStableMemoryJudgeDevelopment(
	ctx context.Context,
	stdout io.Writer,
	options commandOptions,
	startedAt time.Time,
	...
	captured, err := memorycapture.CaptureTransportStableMemoryJudgeDevelopment(
		ctx,
		adminDB,
		runtimeDB,
		options.runID,
	...
	report, reportBody, err :=
		memorycapture.BuildTransportStableMemoryJudgeDevelopmentReport(
			protected.Pool,
			captured,
			authority,
			cost,
	...
	captureID, err := newCaptureID()
	if err != nil {
		return errors.New("create transport-stable Memory Judge capture ID failed")
	}
	artifacts := []memorycapture.Artifact{{
		Name: memorycapture.TransportStableMemoryJudgeArtifactName,
		Body: reportBody,
	}}
	_, manifestBody, err := memorycapture.BuildTransportStableMemoryJudgeRunManifest(
		options.runID,
		captureID,
		options.providerMode,
		startedAt,
	...
	summary := commandSummary{
		SchemaVersion:     "neo-chat.memory-regression-native-summary.v7",
		RunID:             options.runID,
		CaptureID:         captureID,
		CorpusClass:       report.CorpusClass,
		AdmissionMode:     report.AdmissionMode,
	...
	if err := json.NewEncoder(stdout).Encode(summary); err != nil {
		return errors.New("write transport-stable Memory Judge summary failed")
	}
	if !report.Passed {
		return errMetricsFailed
	}
...
func runBufferedMemoryJudgeDevelopment(
	ctx context.Context,
	stdout io.Writer,
	options commandOptions,
	startedAt time.Time,
	...
	if err != nil {
		return errors.Join(errors.New("select buffered Memory Judge split failed"), err)
	}
	index, err := memorycapture.BuildFixtureIndex(selectedPool)
	if err != nil {
		return errors.Join(errors.New("index buffered Memory Judge fixtures failed"), err)
	}
	seed, err := memorycapture.SeedEphemeralDatabase(
		ctx,
		adminDB,
	...
	if err != nil {
		return errors.Join(errors.New("seed buffered Memory Judge database failed"), err)
	}
	if len(seed.Cases) != 300 || providers.judge == nil {
		return memorycapture.ErrCaptureInvalid
	}
	if _, err := memorycapture.PopulateProjectionVectors(
		...
		return errors.Join(errors.New("populate buffered Memory Judge projections failed"), err)
	}
	captured, err := memorycapture.CaptureBufferedMemoryJudgeDevelopment(
		ctx,
		adminDB,
		runtimeDB,
		options.runID,
	...
	if err != nil {
		return errors.Join(errors.New("capture buffered Memory Judge Development failed"), err)
	}
	report, reportBody, err := memorycapture.BuildBufferedMemoryJudgeDevelopmentReport(
		protected.Pool,
		captured,
		authority,
		cost,
	...
	if err != nil {
		return errors.Join(errors.New("build buffered Memory Judge report failed"), err)
	}
	if options.pretty {
		reportBody, err = marshalJSON(report, true)
		if err != nil {
	...
	captureID, err := newCaptureID()
	if err != nil {
		return errors.New("create buffered Memory Judge capture ID failed")
	}
	artifacts := []memorycapture.Artifact{{
		Name: memorycapture.BufferedMemoryJudgeArtifactName,
		Body: reportBody,
	}}
	_, manifestBody, err := memorycapture.BuildBufferedMemoryJudgeRunManifest(
		options.runID,
		captureID,
		options.providerMode,
		startedAt,
	...
	if err != nil {
		return errors.Join(errors.New("build buffered Memory Judge manifest failed"), err)
	}
	artifacts = append(artifacts, memorycapture.Artifact{
		Name: "run-manifest.json",
		Body: manifestBody,
	...
	summary := commandSummary{
		SchemaVersion:     "neo-chat.memory-regression-native-summary.v9",
		RunID:             options.runID,
		CaptureID:         captureID,
		CorpusClass:       report.CorpusClass,
		AdmissionMode:     report.AdmissionMode,
	...
	if err := json.NewEncoder(stdout).Encode(summary); err != nil {
		return errors.New("write buffered Memory Judge summary failed")
	}
	if !report.Passed {
		return errMetricsFailed
	}
...
func runProductionBufferedMemoryJudgeValidation(
	ctx context.Context,
	stdout io.Writer,
	options commandOptions,
	startedAt time.Time,
	...
	captured, err := memorycapture.CaptureProductionBufferedMemoryJudgeValidation(
		ctx,
		adminDB,
		runtimeDB,
		options.runID,
	...
	report, reportBody, err := memorycapture.BuildProductionBufferedMemoryJudgeValidationReport(
		protected.Pool,
		captured,
		config,
		authority,
	...
	captureID, err := newCaptureID()
	if err != nil {
		return errors.New("create production buffered Memory Judge Validation capture ID failed")
	}
	artifacts := []memorycapture.Artifact{{
		Name: memorycapture.ProductionBufferedMemoryJudgeValidationArtifactName,
		Body: reportBody,
	}}
	_, manifestBody, err := memorycapture.BuildProductionBufferedMemoryJudgeValidationRunManifest(
		options.runID,
		captureID,
		options.providerMode,
		startedAt,
	...
	summary := commandSummary{
		SchemaVersion:     "neo-chat.memory-regression-native-summary.v10",
		RunID:             options.runID,
		CaptureID:         captureID,
		CorpusClass:       report.CorpusClass,
		AdmissionMode:     report.AdmissionMode,
...
func wrapAccuracyFirstProviderBundle(
	...
	} else if options.captureMode == memorycapture.CaptureModeProductionMemoryJudgeValidation {
		passage, hybrid, judge, _, err =
	...
	} else if options.captureMode == memorycapture.CaptureModeBufferedMemoryJudge {
		passage, hybrid, judge, _, err =
			memorycapture.WrapBufferedMemoryJudgeDevelopmentProviders(
				options.providerMode,
				bundle.passage,
				bundle.hybrid,
				bundle.judge,
	...
	} else if options.captureMode == memorycapture.CaptureModeJudgeFailureDiagnostic {
		passage, hybrid, judge, _, err =
