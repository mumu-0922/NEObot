...
func (r *PostgresProviderConfigRepository) CommitVoiceProviderConnection(
	ctx context.Context,
...
SET config = jsonb_set(
        jsonb_set(
          jsonb_set(config, '{connectionTestSha256}', to_jsonb($10::text), true),
          '{connectionTestedAt}', to_jsonb($11::text), true
        ),
        '{enabled}', to_jsonb($12::boolean), true
      ),
    updated_at = now()
WHERE id = $1
  AND user_id = $2
  ...
  AND COALESCE(config->>'voiceProvider', '') = $5
  AND COALESCE(config->>'baseUrl', '') = $6
  AND COALESCE(config->>'voiceModel', '') = $7
  AND COALESCE(config->>'voiceId', '') = $8
  AND COALESCE((config->>'enabled')::boolean, false) = $9
  AND $10 <> ''
RETURNING id::text, user_id::text, provider_id, label, encrypted_secret_ref, config
`,
		input.ID,
		...
		input.ProviderID,
		input.ExpectedEncryptedSecretRef,
		input.ExpectedVoiceProvider,
		input.ExpectedBaseURL,
		input.ExpectedVoiceModel,
		input.ExpectedVoiceID,
		input.ExpectedEnabled,
		input.ConnectionTestSHA256,
		testedAt,
		input.Enabled,
