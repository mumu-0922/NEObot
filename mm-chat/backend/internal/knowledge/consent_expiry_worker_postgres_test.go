...
func TestPostgresConsentExpiryWorkersMaterializeExactlyOnce(t *testing.T) {
	...
	manifest := GovernanceManifest{Processor: "mixed", EndpointID: "hosted-main", ModelID: "model-stable-20260712", ModelAPIVersion: "api-20260623",
...
func TestPostgresConsentExpiryOutboxFailureRollsBackMarkerAndRevision(t *testing.T) {
	...
	manifest := GovernanceManifest{Processor: "mixed", EndpointID: "hosted-main", ModelID: "model-stable-20260712", ModelAPIVersion: "api-20260623", AllowedPurposes: []string{"parse"}, AllowedDataTypes: []string{"text/plain"}, Region: "global", RetentionPolicy: "none", DeletionContract: "delete", TrainingUse: "disabled"}
	if _, err := NewGovernanceService(NewPostgresRepository(db)).Apply(ctx, manifest); err != nil {
		t.Fatal(err)
	}
	actorCtx := auth.WithUser(ctx, auth.User{ID: userID})
...
func TestPostgresConsentMutationMaterializesElapsedExpiryFirst(t *testing.T) {
	...
	manifest := GovernanceManifest{Processor: "mixed", EndpointID: "hosted-main", ModelID: "model-stable-20260712", ModelAPIVersion: "api-20260623", AllowedPurposes: []string{"parse", "query_embedding"}, AllowedDataTypes: []string{"text/plain"}, Region: "global", RetentionPolicy: "none", DeletionContract: "delete", TrainingUse: "disabled"}
	if _, err := NewGovernanceService(NewPostgresRepository(db)).Apply(ctx, manifest); err != nil {
		t.Fatal(err)
	}
	actorCtx := auth.WithUser(ctx, auth.User{ID: userID})
