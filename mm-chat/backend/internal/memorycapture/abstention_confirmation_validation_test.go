...
func TestAbstentionConfirmationValidationProfileIsSchemaV21AndHistoricalJSONOmitsFields(
	t *testing.T,
) {
	...
	protected := abstentionConfirmationValidationProtected(pool)
	cost := abstentionConfirmationValidationTestCostBasis()
	costSHA256, err := CostBasisSHA256(cost)
	if err != nil {
		t.Fatal(err)
	}
	config, err := BuildAbstentionConfirmationValidationProfileConfig(
		protected,
		costSHA256,
	...
	execution, err := AbstentionConfirmationValidationExecutionPolicy(ProviderModeFakeProtocol)
	if err != nil {
		t.Fatal(err)
	}
	if config.SchemaVersion != "neo-chat.memory-regression-profile-config.v21" ||
	...
	for index, build := range historicalBuilders {
		...
		for _, field := range []string{
			...
			if bytes.Contains(body, []byte(field)) {
				t.Fatalf("historical profile[%d] gained schema-v21 field %q", index, field)
			}
		}
	}
}

func TestAbstentionConfirmationValidationFakeEvidenceNeverPassesAndReplaysDeterministically(
	t *testing.T,
) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
	...
	profile := abstentionConfirmationValidationProfile(t, pool, config, cost, ProviderModeFakeProtocol)
	report, firstBody, err := BuildAbstentionConfirmationValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	}
	_, secondBody, err := BuildAbstentionConfirmationValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	...
	buildManifest := func() (AbstentionConfirmationValidationRunManifest, []byte, error) {
		return BuildAbstentionConfirmationValidationRunManifest(
			...
			costSHA256,
			report,
			[]Artifact{{Name: AbstentionConfirmationValidationArtifactName, Body: firstBody}},
		)
	}
	...
	if !bytes.Equal(firstManifestBody, secondManifestBody) || manifest.Passed ||
		...
		manifest.CaptureMode != CaptureModeAbstentionConfirmationValidation ||
...
func TestAbstentionConfirmationValidationLivePassAndTerminalFailureSemantics(t *testing.T) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
		t.Fatal(err)
	}
	...
	profile := abstentionConfirmationValidationProfile(t, pool, config, cost, ProviderModeLiveSiliconFlow)
	report, _, err := BuildAbstentionConfirmationValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	...
	failedReport, _, err := BuildAbstentionConfirmationValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
...
func TestAbstentionConfirmationValidationRejectsConfirmationRetryAndCostDrift(t *testing.T) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
		t.Fatal(err)
	}
	...
	profile := abstentionConfirmationValidationProfile(
	...
	profile.ProviderAttempts.JudgeLatency =
		testAccuracyFirstLatency(profile.ProviderAttempts.JudgeAttempts)
	if _, _, err := BuildAbstentionConfirmationValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	); err != nil {
...
func TestAbstentionConfirmationValidationFailureActionPrecedenceIsFrozen(t *testing.T) {
...
func TestAbstentionConfirmationValidationCostAuthorityIsSchemaSeparated(t *testing.T) {
	cost := abstentionConfirmationValidationTestCostBasis()
	if _, err := CostBasisSHA256(cost); err != nil {
		t.Fatal(err)
	}
	if err := ValidateAbstentionConfirmationValidationCostAuthority(
		cost,
		FixedMemoryJudgeAuthority(),
	); err != nil {
		t.Fatal(err)
	...
	for _, legacyValidator := range []func(CostBasis, ConfiguredCandidateJudgeProfileAuthority) error{
		...
		if err := legacyValidator(cost, FixedMemoryJudgeAuthority()); err == nil {
			t.Fatal("historical cost validator accepted schema-v21 authority")
		}
	}
	invalid := cost
	authority := *cost.ConfiguredCandidateJudgeAuthority
	invalid.ConfiguredCandidateJudgeAuthority = &authority
	invalid.ConfiguredCandidateJudgeAuthority.RequestCount = 601
	if err := ValidateAbstentionConfirmationValidationCostAuthority(
		invalid,
		FixedMemoryJudgeAuthority(),
	); err == nil {
		t.Fatal("production Validation accepted request authority above 600")
	}
	for name, mutate := range map[string]func(*CostBasis){
		...
		t.Run(name, func(t *testing.T) {
			candidate := abstentionConfirmationValidationTestCostBasis()
			mutate(&candidate)
			if err := ValidateAbstentionConfirmationValidationCostAuthority(
				candidate, FixedMemoryJudgeAuthority(),
			); err == nil {
				t.Fatal("drifted schema-v21 cost authority was accepted")
			}
		})
	}
}

func abstentionConfirmationValidationProtected(pool memoryauthor.RegressionPool) ProtectedRegression {
...
func abstentionConfirmationValidationProfile(
	t *testing.T,
	...
	t.Helper()
	traces := passingAbstentionConfirmationValidationTraces(pool)
	...
	logicalRequests, logicalInputTokens := abstentionConfirmationValidationLogicalJudgeTelemetry(traces)
	...
	telemetry := abstentionConfirmationValidationTelemetry(
		logicalRequests,
		logicalInputTokens,
		0,
		0,
	...
	telemetry.JudgeAttempts++
	telemetry.JudgeConfirmationAttempts = 1
	telemetry.JudgeConfirmationInputTokenUpperBound = confirmationInputTokens
...
func passingAbstentionConfirmationValidationTraces(
	pool memoryauthor.RegressionPool,
...
func abstentionConfirmationValidationLogicalJudgeTelemetry(
...
func abstentionConfirmationValidationTelemetry(
...
func abstentionConfirmationValidationTestCostBasis() CostBasis {
	authority := FixedMemoryJudgeAuthority()
	return CostBasis{
		SchemaVersion:      "neo-chat.memory-regression-cost-basis.v21",
		...
		ConfiguredCandidateJudgeAuthority: &ConfiguredCandidateJudgeCostAuthority{
			...
			RequestCount: 600, MaximumInputTokens: 600_000,
...
func TestAbstentionConfirmationValidationHashesAreStableHex(t *testing.T) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
		t.Fatal(err)
	}
	values := []func() (string, error){
		...
		abstentionConfirmationProductionRelevancePolicySHA256,
	}
