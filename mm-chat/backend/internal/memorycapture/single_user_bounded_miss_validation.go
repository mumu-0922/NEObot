...
const (
	SingleUserBoundedMissValidationReportSchemaVersion = "neo-chat.memory-regression-relevance-validation.v25-single-user-bounded-miss.v1"
	SingleUserBoundedMissValidationRunSchemaVersion    = "neo-chat.memory-regression-relevance-validation-run.v25-single-user-bounded-miss.v1"
	SingleUserBoundedMissValidationAdmissionMode       = "frozen_production_fixed_memory_judge_negative_guard_double_confirmation_single_user_bounded_miss_validation_only"
	SingleUserBoundedMissValidationArtifactName        = "fixed-memory-judge-negative-guard-double-confirmation-single-user-bounded-miss-production-validation.json"
	singleUserBoundedMissValidationReasonInjection     = "FALSE_INJECTION_NON_ZERO"
)

// SingleUserBoundedMissValidationReport is aggregate-only frozen Validation
// evidence. Passing does not grant Release or promotion authority.
type SingleUserBoundedMissValidationReport struct {
...
type SingleUserBoundedMissValidationRunManifest struct {
	SchemaVersion                   string                      `json:"schemaVersion"`
...
func BuildSingleUserBoundedMissValidationReport(
	pool memoryauthor.RegressionPool,
	profile CapturedProfile,
	config ProfileConfig,
	authority ConfiguredCandidateJudgeProfileAuthority,
	costBasis CostBasis,
) (SingleUserBoundedMissValidationReport, []byte, error) {
	configurationSHA256, err := ConfigurationSHA256(config)
	if err != nil {
		return SingleUserBoundedMissValidationReport{}, nil, err
	}
	...
	criteria, err := memoryeval.MemoryJudgeSingleUserBoundedMissCriteriaV4(pool.Corpus.Criteria)
	if err != nil {
		return SingleUserBoundedMissValidationReport{}, nil, ErrCaptureInvalid
	}
	criteriaSHA256, err := singleUserBoundedMissCriteriaSHA256(pool.Corpus.Criteria)
	if err != nil {
		return SingleUserBoundedMissValidationReport{}, nil, err
	}
	policySHA256, err := singleUserBoundedMissProductionRelevancePolicySHA256()
	if err != nil {
		return SingleUserBoundedMissValidationReport{}, nil, err
	}
	...
	if profile.Profile.ReaderVersion != SingleUserBoundedMissValidationReaderVersion ||
		...
		!validSingleUserBoundedMissValidationConfig(
			config,
			providerMode,
			authority,
			costBasisSHA256,
		...
		ValidateSingleUserBoundedMissValidationCostAuthority(costBasis, authority) != nil ||
	...
	for _, trace := range profile.Calibration {
		if trace.FullObservation.HardCutoffApplied ||
			...
			return SingleUserBoundedMissValidationReport{}, nil, fmt.Errorf(
	...
	outcome := singleUserBoundedMissValidationOutcome(
		evidenceClass,
		evaluation,
	...
	report := SingleUserBoundedMissValidationReport{
		SchemaVersion:                   SingleUserBoundedMissValidationReportSchemaVersion,
		CorpusClass:                     memoryeval.RegressionCorpusClass,
		AdmissionMode:                   SingleUserBoundedMissValidationAdmissionMode,
	...
	if !validSingleUserBoundedMissValidationReport(report) {
		return SingleUserBoundedMissValidationReport{}, nil, ErrCaptureInvalid
	}
	body, err := json.Marshal(report)
	if err != nil {
		return SingleUserBoundedMissValidationReport{}, nil, errors.Join(ErrCaptureInvalid, err)
	}
	return report, append(body, '\n'), nil
}

func BuildSingleUserBoundedMissValidationRunManifest(
	runID string,
	captureID string,
	providerMode string,
	startedAt time.Time,
	...
	report SingleUserBoundedMissValidationReport,
	artifacts []Artifact,
) (SingleUserBoundedMissValidationRunManifest, []byte, error) {
	if !validSingleUserBoundedMissValidationReport(report) ||
		...
		return SingleUserBoundedMissValidationRunManifest{}, nil, ErrCaptureInvalid
	}
	...
	if err != nil || artifactManifest[0].Name != SingleUserBoundedMissValidationArtifactName {
		return SingleUserBoundedMissValidationRunManifest{}, nil, ErrCaptureInvalid
	}
	manifest := SingleUserBoundedMissValidationRunManifest{
		SchemaVersion:                   SingleUserBoundedMissValidationRunSchemaVersion,
		RunID:                           runID,
		CaptureID:                       captureID,
		CorpusClass:                     memoryeval.RegressionCorpusClass,
		AdmissionMode:                   SingleUserBoundedMissValidationAdmissionMode,
		PromotionEligible:               false,
		ReleaseEligible:                 false,
		CaptureMode:                     CaptureModeSingleUserBoundedMissValidation,
		Split:                           FrozenValidationSplit,
	...
	if err != nil {
		return SingleUserBoundedMissValidationRunManifest{}, nil, errors.Join(ErrCaptureInvalid, err)
	}
	return manifest, append(body, '\n'), nil
}

func validSingleUserBoundedMissValidationConfig(
	config ProfileConfig,
	providerMode string,
	authority ConfiguredCandidateJudgeProfileAuthority,
	costBasisSHA256 string,
	...
	expectedProfileID, err := candidateProfileID(providerMode)
	return err == nil && config.SchemaVersion == "neo-chat.memory-regression-profile-config.v25-single-user-bounded-miss-validation.v1" &&
...
func validSingleUserBoundedMissValidationReport(
	report SingleUserBoundedMissValidationReport,
) bool {
	providerMode, err := providerModeForProfileID(report.ProfileID)
	if err != nil || memoryeval.ValidateMemoryJudgeSingleUserBoundedMissCriteriaV4(
		report.EvaluationCriteria,
	) != nil || memoryeval.ValidateSingleUserBoundedMissCalibrationEvaluation(
		report.Evaluation,
		report.EvaluationCriteria,
	) != nil {
		return false
	...
	if err != nil || report.EvaluationCriteriaSHA256 != criteriaSHA256 {
		return false
	}
	expectedExecution, err := SingleUserBoundedMissValidationExecutionPolicy(providerMode)
	...
	expectedOutcome := singleUserBoundedMissValidationOutcome(
	...
	return report.SchemaVersion == SingleUserBoundedMissValidationReportSchemaVersion &&
		report.CorpusClass == memoryeval.RegressionCorpusClass &&
		report.AdmissionMode == SingleUserBoundedMissValidationAdmissionMode &&
...
func singleUserBoundedMissProductionRelevancePolicySHA256() (string, error) {
	return relevancePolicyDescriptorSHA256(
		usermemory.HybridShadowDoubleConfirmationProductionPolicy(),
	)
}

func singleUserBoundedMissValidationOutcome(
	...
	if outcome.Severity == ProductionValidationSeverityOrange &&
		(evaluation.Metrics.FalseInjectionCases > 0 ||
			evaluation.Metrics.FalseInjectionRate > 0) {
		outcome.Reasons = []string{singleUserBoundedMissValidationReasonInjection}
	}
	return outcome
}
