...
func TestProductionBufferedMemoryJudgeValidationProfileIsSchemaV18AndHistoricalJSONOmitsFields(
	t *testing.T,
) {
	...
	cost := productionBufferedMemoryJudgeValidationTestCostBasis()
	costSHA256, err := CostBasisSHA256(cost)
	if err != nil {
		t.Fatal(err)
	}
	config, err := BuildProductionBufferedMemoryJudgeValidationProfileConfig(
		protected,
		costSHA256,
	...
	execution, err := ProductionBufferedMemoryJudgeValidationExecutionPolicy(ProviderModeFakeProtocol)
	if err != nil {
		t.Fatal(err)
	}
	if config.SchemaVersion != "neo-chat.memory-regression-profile-config.v18" ||
	...
	for index, build := range historicalBuilders {
		...
		for _, field := range []string{
			...
			if bytes.Contains(body, []byte(field)) {
				t.Fatalf("historical profile[%d] gained schema-v18 field %q", index, field)
			}
		}
	}
}

func TestProductionBufferedMemoryJudgeValidationFakeEvidenceNeverPassesAndReplaysDeterministically(
	t *testing.T,
) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
	...
	profile := productionBufferedValidationProfile(t, pool, config, cost, ProviderModeFakeProtocol)
	report, firstBody, err := BuildProductionBufferedMemoryJudgeValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	}
	_, secondBody, err := BuildProductionBufferedMemoryJudgeValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	...
	buildManifest := func() (ProductionBufferedMemoryJudgeValidationRunManifest, []byte, error) {
		return BuildProductionBufferedMemoryJudgeValidationRunManifest(
			...
			costSHA256,
			report,
			[]Artifact{{Name: ProductionBufferedMemoryJudgeValidationArtifactName, Body: firstBody}},
		)
	}
	...
	if !bytes.Equal(firstManifestBody, secondManifestBody) || manifest.Passed ||
		...
		manifest.CaptureMode != CaptureModeProductionBufferedMemoryJudgeValidation ||
...
func TestProductionBufferedMemoryJudgeValidationLivePassAndTerminalFailureSemantics(t *testing.T) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
		t.Fatal(err)
	}
	...
	profile := productionBufferedValidationProfile(t, pool, config, cost, ProviderModeLiveSiliconFlow)
	report, _, err := BuildProductionBufferedMemoryJudgeValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
	...
	logicalRequests, logicalInputTokens := productionBufferedValidationLogicalJudgeTelemetry(profile.Calibration)
	profile.ProviderAttempts = productionBufferedValidationTelemetry(
		logicalRequests,
		logicalInputTokens,
		2,
	...
	failedReport, _, err := BuildProductionBufferedMemoryJudgeValidationReport(
		pool, profile, config, FixedMemoryJudgeAuthority(), cost,
	)
	if err != nil {
		t.Fatal(err)
...
func TestProductionBufferedValidationFailureActionPrecedenceIsFrozen(t *testing.T) {
...
func TestProductionBufferedMemoryJudgeValidationCostAuthorityIsSchemaSeparated(t *testing.T) {
	cost := productionBufferedMemoryJudgeValidationTestCostBasis()
	if _, err := CostBasisSHA256(cost); err != nil {
		t.Fatal(err)
	}
	if err := ValidateProductionBufferedMemoryJudgeValidationCostAuthority(
		cost,
		FixedMemoryJudgeAuthority(),
	); err != nil {
		t.Fatal(err)
	...
	for _, legacyValidator := range []func(CostBasis, ConfiguredCandidateJudgeProfileAuthority) error{
		...
		if err := legacyValidator(cost, FixedMemoryJudgeAuthority()); err == nil {
			t.Fatal("historical cost validator accepted schema-v13 authority")
		}
	}
	invalid := cost
...
func productionBufferedValidationProtected(pool memoryauthor.RegressionPool) ProtectedRegression {
...
func productionBufferedValidationProfile(
	t *testing.T,
	...
	t.Helper()
	traces := passingBufferedCloudJudgeValidationTraces(pool)
	logicalRequests, logicalInputTokens := productionBufferedValidationLogicalJudgeTelemetry(traces)
	...
	return CapturedProfile{
		...
		Cases:       cases,
		Calibration: traces,
		ProviderAttempts: productionBufferedValidationTelemetry(
			logicalRequests,
			logicalInputTokens,
			0,
			0,
...
func passingBufferedCloudJudgeValidationTraces(
	pool memoryauthor.RegressionPool,
...
func productionBufferedValidationLogicalJudgeTelemetry(
...
func productionBufferedValidationTelemetry(
...
func productionBufferedMemoryJudgeValidationTestCostBasis() CostBasis {
	authority := FixedMemoryJudgeAuthority()
	return CostBasis{
		SchemaVersion:      "neo-chat.memory-regression-cost-basis.v13",
...
func TestProductionBufferedMemoryJudgeValidationHashesAreStableHex(t *testing.T) {
	pool, err := memoryauthor.GenerateRegressionV5()
	if err != nil {
		t.Fatal(err)
	}
	values := []func() (string, error){
		...
		productionBufferedRelevancePolicySHA256,
	}
