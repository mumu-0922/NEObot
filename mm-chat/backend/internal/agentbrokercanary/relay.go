// Package agentbrokercanary composes the G21.2 synthetic action plan with the
// durable Broker authority. It is not imported by the public API or Chat path.
...
func (target *RelayTarget) Prepare(ctx context.Context, request agentrunner.PrepareRequest) (agentrunner.PrepareResult, error) {
	...
	prepared, err := target.broker.Prepare(ctx, agentbroker.PrepareInput{
		...
		Attempt:   attemptAuthority(binding.UserID, request.Attempt, request.Authority, request.SnapshotFingerprint, request.GrantFingerprint, request.RegistryFingerprint),
		Grant:     binding.Grant, Registry: binding.Registry,
		ToolIdentity: request.ToolIdentity, Action: request.Action, Resource: request.Resource,
		Arguments: request.Arguments, BaseRevision: request.BaseRevision,
...
func (target *RelayTarget) Commit(ctx context.Context, request agentrunner.CommitRequest) (agentrunner.CommitResult, error) {
	...
	result, err := target.broker.Commit(ctx, agentbroker.CommitInput{
		Attempt: attemptAuthority(userID, request.Attempt, request.Authority, request.SnapshotFingerprint,
			request.GrantFingerprint, request.RegistryFingerprint),
