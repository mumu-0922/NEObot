...
var (
	ErrUnavailable     = errors.New("BROKER_CANARY_UNAVAILABLE")
	ErrRecoveryPending = errors.New("BROKER_CANARY_RECOVERY_PENDING")
)
...
func NewService(config Config, plan Plan, bindings *Bindings, gate Gate, orchestrator Orchestrator,
	...
	if config.RunnerID == "" || config.CallerIdentity != BrokerCanaryIdentity ||
		...
		bindings.runnerID != config.RunnerID || len(bindings.All()) != 5 || gate == nil ||
		orchestrator == nil || authority == nil || runnerState == nil || client == nil {
	...
	return &Service{config: config, plan: plan, bindings: bindings, gate: gate,
		...
		client: client, now: time.Now}, nil
}
...
func (service *Service) Cycle(ctx context.Context) (CycleResult, error) {
	...
	result := CycleResult{}
	for _, binding := range service.bindings.All() {
...
func (service *Service) runAction(ctx context.Context, binding ActionBinding, probeFingerprint string) (bool, bool, error) {
	...
	actor := agentorchestrator.Actor{Type: "orchestrator", ID: "g21.2-broker-canary"}
	...
	lease, err := service.orchestrator.AcquireStep(ctx, agentorchestrator.AcquireInput{
		...
		ReasonCode: "BROKER_CANARY_CLAIMED"})
	if err != nil {
		return false, false, ErrUnavailable
	}
	...
	transition.ReasonCode = "BROKER_CANARY_STARTING"
	...
	grantFingerprint := binding.SnapshotGrantFingerprint()
	...
	transition.Expected, transition.To, transition.ReasonCode = agentorchestrator.AttemptStarting,
		agentorchestrator.AttemptRunning, "BROKER_CANARY_RUNNING"
	if service.orchestrator.TransitionAttempt(ctx, transition) != nil {
		return false, false, ErrUnavailable
	}
	...
	if _, err := service.orchestrator.HeartbeatAttempt(ctx, agentorchestrator.HeartbeatInput{
		...
		LeaseDuration: leaseDuration, Actor: actor, ReasonCode: "BROKER_CANARY_HEARTBEAT"}); err != nil {
		return false, false, ErrUnavailable
	}
	...
	if !ok || !prepared.Prepared || prepared.IntentID == "" || prepared.IntentFingerprint == "" ||
		prepared.IdempotencyKey == "" || prepared.Approval != "automatic" {
		return false, false, ErrUnavailable
	}
	...
	cancelRequest, err := service.authorizedRequest(ctx, agentrunner.MethodCancel,
		agentrunner.CancelRequest{Attempt: attempt, Mode: "cancel", ReasonCode: "broker_canary_complete"},
		binding.SnapshotFingerprint, attempt, service.now().UTC())
	if err != nil {
		return false, false, err
	}
	...
	if !ok || (commitErr != nil && !errors.Is(commitErr, agentrunner.ErrOutcomeUnknown)) ||
		(committed.Outcome != "committed" && committed.Outcome != "outcome_unknown") {
		return false, false, ErrUnavailable
	}
