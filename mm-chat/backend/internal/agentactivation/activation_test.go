...
func TestVerifyProjectCanaryRequiresMigration093ApprovalAndNarrowMutation(t *testing.T) {
...
func activationFixture(t *testing.T, now time.Time) (Config, record) {
	...
	policyRaw := []byte(`{"schemaVersion":"neo.agent-production-policy/v1","migrationHead":93}`)
	...
	value := record{
		...
		Release: release{GitCommit: strings.Repeat("a", 40), MigrationHead: 93,
...
func brokerCanaryActivationFixture(t *testing.T, now time.Time) (BrokerCanaryConfig, brokerCanaryRecord) {
	...
	control.Release.MigrationHead = 93
