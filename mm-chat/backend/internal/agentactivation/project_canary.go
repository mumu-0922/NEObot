...
const (
	StageProjectCanary          = "project_mutation_canary"
	ProjectCanaryCallerIdentity = "spiffe://neo-chat/agent-runtime-project-canary"
	ProjectRunnerRelayIdentity  = "spiffe://neo-chat/neo-runner-project-relay"
)

var projectCanaryChecks = []string{
...
type ProjectCanaryConfig struct {
	Config
	CanaryPlanFile             string
	AuthorityPublicKeyFile     string
	ApprovalDocumentFile       string
...
type projectCanaryWiring struct {
	...
	ApprovalDocumentSHA256       string `json:"approvalDocumentSha256"`
	ApprovalPublicKeySHA256      string `json:"approvalPublicKeySha256"`
...
type projectCanaryAuthorization struct {
...
type projectCanaryCleanup struct {
	...
	ProjectResidue    int `json:"projectResidue"`
}

type projectCanaryRecord struct {
	SchemaVersion string                     `json:"schemaVersion"`
	EvidenceClass string                     `json:"evidenceClass"`
	Stage         string                     `json:"stage"`
	Release       release                    `json:"release"`
	Target        target                     `json:"target"`
	Wiring        projectCanaryWiring        `json:"wiring"`
	Window        window                     `json:"window"`
	Checks        []check                    `json:"checks"`
	Authorization projectCanaryAuthorization `json:"authorization"`
	Cleanup       projectCanaryCleanup       `json:"cleanup"`
	Review        review                     `json:"review"`
}

func VerifyProjectCanary(config ProjectCanaryConfig, now time.Time) (Decision, error) {
	if validateProjectCanaryConfig(config) != nil || now.IsZero() {
	...
	var evidence projectCanaryRecord
	if validateProjectCanaryShape(recordRaw) != nil || strictjson.Decode(recordRaw, maxDocument, &evidence) != nil {
		decision.ReasonCode = "ACTIVATION_ROOT_INVALID"
		return decision, ErrInvalid
	}
	if reason := validateProjectCanaryRecord(evidence, config, policyFingerprint, now.UTC()); reason != "" {
	...
	bindings := []struct {
		...
		{config.ApprovalDocumentFile, evidence.Wiring.ApprovalDocumentSHA256, "APPROVAL_DOCUMENT_DRIFT"},
		{config.ApprovalPublicKeyFile, evidence.Wiring.ApprovalPublicKeySHA256, "APPROVAL_KEY_DRIFT"},
	...
	if _, err := agentrunner.LoadEd25519PublicKey(config.ApprovalPublicKeyFile); err != nil {
		decision.ReasonCode = "APPROVAL_KEY_INVALID"
		return decision, ErrInvalid
	}
	decision.Ready, decision.ReasonCode = true, "PROJECT_MUTATION_CANARY_GATES_PASSED"
	return decision, nil
}

func validateProjectCanaryShape(raw []byte) error {
	...
	for key, fields := range map[string][]string{
		...
		"wiring": {"endpointSha256", "clientCertificateSha256", "serverCASha256", "serverName", "callerIdentity",
			"canaryPlanSha256", "authorityPublicKeySha256", "approvalDocumentSha256", "approvalPublicKeySha256", "relayEndpointSha256",
			"relayServerCertificateSha256", "relayClientCASha256", "runnerRelayIdentity"},
		"window": {"startedAt", "completedAt", "expiresAt"},
		"authorization": {"controlPlane", "rootRuns", "brokerReadOnly", "artifactPublication", "projectMutation",
			"brokerMutable", "delegation", "scheduler", "learning"},
		"cleanup": {"orphanSandboxes", "scratchResidue", "artifactResidue", "quarantineResidue", "projectResidue"},
	...
	var checks []any
	if strictjson.Decode(root["checks"], maxDocument, &checks) != nil || len(checks) != len(projectCanaryChecks) {
		return ErrInvalid
	}
	return nil
}

func validateProjectCanaryRecord(value projectCanaryRecord, config ProjectCanaryConfig,
	policyFingerprint string, now time.Time,
) string {
	if value.SchemaVersion != SchemaVersion || value.Stage != StageProjectCanary ||
	...
	if !validFingerprint(value.Target.DeploymentFingerprint) || value.Target.DeploymentFingerprint != config.TargetFingerprint ||
		value.Target.RunnerID != config.RunnerID ||
	...
	fingerprints := []string{value.Release.RunnerManifestSHA256, value.Release.RunnerBinarySHA256,
		...
		value.Wiring.ApprovalDocumentSHA256, value.Wiring.ApprovalPublicKeySHA256,
	...
	if !validFingerprint(value.Wiring.EndpointSHA256) || !validFingerprint(value.Wiring.ClientCertificateSHA256) ||
		...
		!validFingerprint(value.Wiring.ApprovalDocumentSHA256) || !validFingerprint(value.Wiring.ApprovalPublicKeySHA256) ||
		...
		value.Wiring.RelayEndpointSHA256 != ProjectRelayEndpointFingerprint(config.RelayEndpoint) ||
	...
	if value.Wiring.AuthorityPublicKeySHA256 == value.Wiring.ApprovalPublicKeySHA256 ||
		value.Wiring.ClientCertificateSHA256 == value.Wiring.RelayServerCertificateSHA256 ||
		value.Wiring.ApprovalDocumentSHA256 == value.Wiring.ApprovalPublicKeySHA256 {
		return "AUTHORITY_REUSE_FORBIDDEN"
	}
	...
	for _, item := range value.Checks {
		if !contains(projectCanaryChecks, item.ID) || results[item.ID] != "" ||
	...
	sort.Strings(ids)
	if !sameStrings(ids, projectCanaryChecks) {
	...
	if value.Authorization.ControlPlane || !value.Authorization.RootRuns || !value.Authorization.BrokerReadOnly ||
		!value.Authorization.ArtifactPublication || !value.Authorization.ProjectMutation || value.Authorization.BrokerMutable ||
	...
	if cleanup.OrphanSandboxes < 0 || cleanup.ScratchResidue < 0 || cleanup.ArtifactResidue < 0 || cleanup.ProjectResidue < 0 ||
		cleanup.QuarantineResidue < 0 || cleanup.OrphanSandboxes > 1_000_000 || cleanup.ScratchResidue > 1_000_000 ||
		cleanup.ArtifactResidue > 1_000_000 || cleanup.QuarantineResidue > 1_000_000 || cleanup.ProjectResidue > 1_000_000 {
	...
	if cleanup.OrphanSandboxes != 0 || cleanup.ScratchResidue != 0 ||
		cleanup.ArtifactResidue != 0 || cleanup.QuarantineResidue != 0 || cleanup.ProjectResidue != 0 {
...
func validateProjectCanaryConfig(config ProjectCanaryConfig) error {
	for _, path := range []string{config.PolicyFile, config.RecordFile, config.ReleaseManifestFile,
		...
		config.AuthorityPublicKeyFile, config.ApprovalDocumentFile, config.ApprovalPublicKeyFile,
		config.RelayServerCertificateFile, config.RelayClientCAFile} {
		if len(path) < 2 || path[0] != '/' {
			return ErrInvalid
		}
	}
	if config.Endpoint == "" || config.RelayEndpoint == "" || !identityPattern.MatchString(config.RunnerID) ||
		!identityPattern.MatchString(config.ServerName) || config.CallerIdentity != ProjectCanaryCallerIdentity ||
		config.RunnerRelayIdentity != ProjectRunnerRelayIdentity || !validFingerprint(config.TargetFingerprint) ||
...
func ProjectRelayEndpointFingerprint(endpoint string) string {
	return fingerprint(append([]byte("neo-agent-project-relay-endpoint-v1\x00"), []byte(endpoint)...))
}
