...
- Generate, semantically audit, publish, and byte-replay the independent v2
  regression corpus plus separately versioned v3/v4/v5 repairs without
  changing the v1 formal-authoring profile or historical bytes.
...
# Universally unrelated hard negative. This preserves v2/v3/v4.
...
`regression-v4-*`, and `regression-v5-*` commands default to matching versioned
roots. The final path component must explicitly contain `regression`,
generation is exclusive, and every artifact is bound to exactly one fixed
profile. Verification dispatches from the protected generator tuple and
rejects unknown or mixed v2/v3/v4/v5 artifacts. A `holdout` case label in any
lane is only a visible regression stratum; it is not a secret or one-shot
formal Holdout.
...
```text
v2-regression/, v3-regression/, v4-regression/, or v5-regression/
...
claims in that candidate. V5 keeps the compatible positives but replaces the
hard negative with a same-entity/same-scope commemorative-mug location. Its
audit rejects every known Subject/current/old value in both languages plus the
v3/v4 meeting, agenda, facilities, weather, and sunshine event families from
that candidate.
Historical v2/v3/v4 bytes and reports remain immutable.
...
| `GenerateRegression` / `GenerateRegressionV3` / `GenerateRegressionV4` / `GenerateRegressionV5` / `AuditRegression` / `ValidateRegressionPool` | Build and machine-audit an exact known regression profile while preserving historical bytes. |
