...
```text
exact known regression profile/seed (legacy v2 or repaired v3/v4/v5)
        |
        v
...
| Separately seeded v5 universal-negative repair | V4's hard negative omitted the selected Subject but still drew from a facilities/weather event family that one live Judge admitted once. | V2/v3/v4 bytes remain frozen; v5 keeps aligned positives, uses a same-entity/scope physical mug-location observation, rejects every known Subject/value and v3/v4 event family, and rejects all mixed-version artifacts. |
...
- All v2, v3, v4, and v5 regression splits are machine-visible. Their `holdout`
...
- 2026-08-01: preserved v2/v3/v4 byte-for-byte and added the separately seeded
  v5 universal-negative repair with exact Subject/value/event-family exclusion.
