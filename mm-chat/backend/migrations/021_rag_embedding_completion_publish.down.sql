-- Revert G7.5P passage-embedding publish finalizer.
...
REVOKE EXECUTE ON FUNCTION knowledge_complete_embedding_and_publish(
  UUID, UUID, UUID, UUID
) FROM rag_worker_executor;
DROP FUNCTION IF EXISTS knowledge_complete_embedding_and_publish(
  UUID, UUID, UUID, UUID
);
REVOKE UPDATE(status, current_version_id, updated_at)
  ON knowledge_documents
FROM rag_projection_owner;
REVOKE UPDATE(status, error_code, updated_at)
