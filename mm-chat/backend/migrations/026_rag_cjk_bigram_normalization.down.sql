-- Roll back locale-independent CJK normalization to migration 025.
--
...
BEGIN
  ...
  FROM (
    SELECT DISTINCT lower(substr(normalized_query, ordinal, 2)) AS term
    FROM generate_series(1, greatest(char_length(normalized_query) - 1, 0))
      AS ordinal
    WHERE substr(normalized_query, ordinal, 2) ~ '^[[:alnum:]]{2}$'
    LIMIT 64
  ) terms;
