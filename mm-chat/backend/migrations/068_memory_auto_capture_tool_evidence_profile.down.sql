-- Restore the byte-identical migration-067 promotion function only before any
-- automatic promotion has created irreversible canonical/audit history.
DO $guard_auto_capture_tool_profile_rollback$
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION USING ERRCODE = '55000',
      MESSAGE = 'MEMORY_AUTO_CAPTURE_TOOL_PROFILE_ROLLBACK_REQUIRES_NO_PROMOTIONS';
  END IF;
END
$guard_auto_capture_tool_profile_rollback$;
...
BEGIN
  ...
  IF v_batch.extraction_profile_id <> encode(sha256(convert_to(
      v_job.processing_profile || chr(31) ||
        'memory-capture-candidate-tool-v3', 'UTF8'
    )), 'hex')
