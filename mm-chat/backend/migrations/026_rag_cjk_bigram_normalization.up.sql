-- G11.9A locale-independent CJK bigram normalization.
--
-- POSIX [:alnum:] follows the database locale and can reject every CJK
-- bigram under C-style locales. Compact whitespace plus ASCII/CJK punctuation
-- first, then generate bounded overlapping bigrams without locale classes.
--
-- PostgreSQL's simple text-search configuration does not segment CJK text, so
-- an exact Chinese phrase such as `研究方向` previously returned no candidates
-- even when the published child contained it. Preserve the existing English
-- lexical/exact-term path and add bounded phrase plus overlapping bigram
-- signals. The function still returns references only; Go must reauthorize and
-- hydrate them before answer generation.
