-- Restore the byte-identical migration-068 promotion function only before any
-- automatic promotion has created irreversible canonical/audit history.
DO $guard_auto_capture_compatible_profile_rollback$
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION USING ERRCODE = '55000',
      MESSAGE = 'MEMORY_AUTO_CAPTURE_COMPATIBLE_PROFILE_ROLLBACK_REQUIRES_NO_PROMOTIONS';
  END IF;
END
$guard_auto_capture_compatible_profile_rollback$;
...
BEGIN
  ...
  -- A committed legacy free-text batch is not authority for the required Tool
  -- protocol. Bind promotion to the exact current prompt versions and reject
  -- any partial or profile-drifted batch before examining candidate content.
  IF v_batch.extraction_profile_id <> encode(sha256(convert_to(
      v_job.processing_profile || chr(31) ||
        'memory-capture-candidate-tool-v4', 'UTF8'
    )), 'hex')
    OR v_batch.decision_profile_id <> encode(sha256(convert_to(
      v_job.processing_profile || chr(31) ||
        'memory-capture-decision-tool-v2', 'UTF8'
    )), 'hex')
    OR (
      SELECT count(*) FROM user_memory_review_suggestions suggestion
    ...
    OR EXISTS (
      SELECT 1 FROM user_memory_review_suggestions suggestion
      WHERE suggestion.capture_job_id = v_job.job_id
        AND suggestion.event_id = v_job.event_id
        AND suggestion.user_id = v_job.user_id
        AND (
          suggestion.extraction_profile_id <> v_batch.extraction_profile_id
          OR suggestion.decision_profile_id <> v_batch.decision_profile_id
        )
    )
  THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001', MESSAGE = 'MEMORY_PROFILE_DRIFT';
  END IF;
  ...
  LOOP
    ...
    ELSIF encode(sha256(convert_to(
        v_suggestion.candidate_content, 'UTF8'
      )), 'hex') <> v_suggestion.candidate_hash
    THEN
      v_review_reason := 'AUTO_PROMOTION_CANDIDATE_DRIFT';
    ELSIF EXISTS (
      SELECT 1
      FROM user_memory_review_evidence evidence
      LEFT JOIN messages evidence_message
        ON evidence_message.id = evidence.source_message_id
        AND evidence_message.user_id = evidence.user_id
      WHERE evidence.suggestion_id = v_suggestion.id
        AND evidence.user_id = v_job.user_id
        AND (
          evidence_message.id IS NULL
          OR evidence_message.conversation_id <>
            evidence.source_conversation_id
          OR evidence.source_conversation_id <>
            v_job.source_conversation_id
          OR evidence_message.status <> 'completed'
          OR evidence_message.deleted_at IS NOT NULL
          OR (
            evidence.evidence_role = 'user'
            AND evidence_message.role <> 'user'
          )
          OR (
            evidence.evidence_role = 'assistant_context'
            AND evidence_message.role <> 'assistant'
          )
          OR encode(sha256(convert_to(
            evidence_message.content, 'UTF8'
          )), 'hex') <> evidence.source_content_hash
          OR COALESCE(
