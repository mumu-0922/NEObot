-- Forward-only OpenAI-compatible Tool-schema profile correction.
-- Migration 068 remains byte-immutable after live application; extraction v5
-- retains per-role evidence enums and bounded arrays without the unsupported
-- JSON Schema uniqueItems keyword.
...
BEGIN
  ...
  IF v_batch.extraction_profile_id <> encode(sha256(convert_to(
      v_job.processing_profile || chr(31) ||
        'memory-capture-candidate-tool-v5', 'UTF8'
    )), 'hex')
