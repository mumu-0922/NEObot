-- Roll back SiliconFlow profile admission only from a clean Jina baseline.
-- Any BGE generation or projection is immutable evidence and must be retired
-- and purged through the operator lifecycle before schema rollback.
SELECT set_config(
  'search_path',
...
DO $guard_bge_rollback$
BEGIN
  IF EXISTS (
    SELECT 1 FROM knowledge_search_profiles
    WHERE provider_profile_id = 'siliconflow_bge_m3_v1'
       OR embedding_processor = 'siliconflow'
       OR embedding_model_id = 'Pro/BAAI/bge-m3'
       OR rerank_processor = 'siliconflow'
       OR rerank_model_id = 'Pro/BAAI/bge-reranker-v2-m3'
  ) OR EXISTS (
    SELECT 1 FROM knowledge_index_profiles
    WHERE embedding_processor = 'siliconflow'
       OR embedding_model_id = 'Pro/BAAI/bge-m3'
       OR rerank_processor = 'siliconflow'
       OR rerank_model_id = 'Pro/BAAI/bge-reranker-v2-m3'
  ) OR EXISTS (
    SELECT 1 FROM knowledge_child_search_projections
...
ALTER FUNCTION knowledge_complete_embedding_and_publish(
  UUID, UUID, UUID, UUID
) OWNER TO rag_projection_owner;
...
DROP FUNCTION knowledge_fetch_fenced_profiled_query_evidence_candidates(
  UUID, UUID, UUID[], TEXT, REAL[], INTEGER
);
DROP FUNCTION knowledge_fetch_fenced_query_evidence_candidates(
  UUID, UUID, UUID[], TEXT, INTEGER
);
DROP FUNCTION knowledge_fetch_fenced_hybrid_candidates_core(
  UUID, UUID, UUID[], TEXT, REAL[], INTEGER
);
DROP FUNCTION knowledge_resolve_active_retrieval_profile();
DROP FUNCTION knowledge_resolve_generation_retrieval_profile(UUID);
DROP FUNCTION knowledge_resolve_generation_embedding_profile(
  UUID, UUID, UUID, UUID, UUID
);

DROP INDEX idx_knowledge_child_vector_shadow_bge_hnsw;
DROP INDEX idx_knowledge_child_vector_shadow_jina_hnsw;
...
ALTER TABLE knowledge_search_profiles
  DROP CONSTRAINT knowledge_search_profiles_supported_tuple_check;
ALTER TABLE knowledge_search_profiles
  ADD CONSTRAINT knowledge_search_profiles_provider_profile_id_check CHECK (
    provider_profile_id = 'mineru_jina_postgres_v1'
  ),
  ADD CONSTRAINT knowledge_search_profiles_embedding_processor_check CHECK (
    embedding_processor = 'jina'
  ),
  ADD CONSTRAINT knowledge_search_profiles_embedding_model_id_check CHECK (
    embedding_model_id = 'jina-embeddings-v4'
  ),
  ADD CONSTRAINT knowledge_search_profiles_rerank_processor_check CHECK (
    rerank_processor = 'jina'
  ),
  ADD CONSTRAINT knowledge_search_profiles_rerank_model_id_check CHECK (
    rerank_model_id = 'jina-reranker-v3'
  );
ALTER TABLE knowledge_child_search_projections
  DROP CONSTRAINT knowledge_child_search_supported_model_check;
ALTER TABLE knowledge_child_search_projections
  ADD CONSTRAINT knowledge_child_search_projections_embedding_model_id_check
  CHECK (embedding_model_id = 'jina-embeddings-v4');
ALTER TABLE knowledge_child_vector_shadow_projections
  DROP CONSTRAINT knowledge_vector_shadow_supported_model_check;
ALTER TABLE knowledge_child_vector_shadow_projections
  ADD CONSTRAINT knowledge_child_vector_shadow_projections_embedding_model_id_check
  CHECK (embedding_model_id = 'jina-embeddings-v4');
...
WHERE processor = 'siliconflow'
  ...
  AND policy_version = '2026-07-24-siliconflow-pro-bge-approved-v1';
DELETE FROM processor_governance_heads
WHERE processor = 'siliconflow'
  AND endpoint_id = 'siliconflow-cn-v1'
  AND model_id IN (
    'Pro/BAAI/bge-m3',
...
DELETE FROM processor_governance_profiles
WHERE id IN (
  'ffd802ab-0e84-5fbb-a6cd-991e96dbe36e'::UUID,
  'a317ecb4-dc6a-5541-bc65-81cccca900e6'::UUID
);
ALTER TABLE processor_governance_profiles
  ENABLE TRIGGER processor_governance_profiles_immutable;
ALTER TABLE knowledge_structure_chunk_profile_descriptors
  DISABLE TRIGGER knowledge_structure_chunk_profile_descriptors_immutable;
DELETE FROM knowledge_structure_chunk_profile_descriptors
WHERE chunk_profile_hash =
  '36845c249aa551d4d86720c38dfef9eb9e36ed49573a7547d2a5381d5f085d73';
ALTER TABLE knowledge_structure_chunk_profile_descriptors
  ENABLE TRIGGER knowledge_structure_chunk_profile_descriptors_immutable;

DROP FUNCTION knowledge_retrieval_profile_id(
