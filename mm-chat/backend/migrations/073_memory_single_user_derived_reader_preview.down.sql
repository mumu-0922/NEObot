DO $memory_single_user_derived_reader_preview_rollback_guard$
BEGIN
  IF to_regclass('memory_single_user_derived_reader_events') IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = '55000',
      MESSAGE = 'MEMORY_SINGLE_USER_DERIVED_READER_PREVIEW_ROLLBACK_REQUIRES_073';
  END IF;
  IF EXISTS (SELECT 1 FROM memory_single_user_derived_reader_events) THEN
    RAISE EXCEPTION USING ERRCODE = '55000',
      MESSAGE = 'MEMORY_SINGLE_USER_DERIVED_READER_PREVIEW_ROLLBACK_REQUIRES_EMPTY_EVENTS';
  END IF;
END
$memory_single_user_derived_reader_preview_rollback_guard$;
...
DROP TRIGGER users_single_user_derived_reader_population_changed ON users;
DROP TRIGGER user_memory_persona_single_user_preview_reconcile
  ON user_memory_persona_versions;
DROP TRIGGER user_memory_scenes_single_user_preview_reconcile
  ON user_memory_scenes;
DROP TRIGGER memory_single_user_derived_reader_events_append_only
  ON memory_single_user_derived_reader_events;
...
DO $pin_memory_single_user_derived_reader_preview_down$
DECLARE
...
BEGIN
  ...
  END LOOP;
END
$pin_memory_single_user_derived_reader_preview_down$;

DROP FUNCTION memory_operator_set_single_user_derived_reader_preview(
  UUID, UUID, BOOLEAN, TEXT
);
DROP FUNCTION memory_single_user_derived_reader_users_changed();
DROP FUNCTION memory_single_user_derived_reader_events_append_only();
DROP FUNCTION memory_single_user_derived_reader_artifact_reconcile();
DROP FUNCTION memory_single_user_derived_reader_preview_enabled(UUID);
