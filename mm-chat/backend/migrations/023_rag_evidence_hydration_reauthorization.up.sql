-- G7.6B Go evidence hydration must bind every Python candidate reference to
-- current Postgres authority, including the returned child content hash.  The
-- function signature stays stable for Go callers; its result gains content_hash
-- so Go can verify the hydrated body matches the reference it accepted.
...
BEGIN
  ...
  SELECT authorized.collection_id, authorized.document_id,
    ...
    authorized.content_hash, authorized.content, authorized.locator_summary
  FROM authorized
