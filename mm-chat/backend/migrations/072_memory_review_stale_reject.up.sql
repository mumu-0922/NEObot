-- Allow a user to discard a current pending Review even when its referenced
-- target Memory changed after proposal. Reject never consumes or mutates the
-- target; all decisions that do consume target state remain revision-fenced.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'MEMORY_REVIEW_STALE_REJECT_REQUIRES_071';
  END IF;
END
...
BEGIN
  ...
  -- Reject discards only the pending candidate. It never reads or mutates a
  -- canonical Memory, so later epoch, scope, or target drift must not strand
  -- the candidate. Every target-consuming decision keeps the original fences.
