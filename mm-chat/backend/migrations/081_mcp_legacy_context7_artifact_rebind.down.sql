...
SET auth_config = jsonb_set(
      server.auth_config #- '{metadata,legacyArtifactRepair}',
      '{metadata,marketplace,deploymentHash}',
      to_jsonb('20a578fff586f03151f2f2f6aa97ea331d3985f8e93ce068f0f59e984cc4964d'::text),
      true
    ),
    updated_at = now()
WHERE server.deleted_at IS NULL
  ...
  AND server.auth_config #>> '{metadata,legacyArtifactRepair}' = '081';
